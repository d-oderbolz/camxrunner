...
c-----------------------------------------------------------------------
c
         subroutine alloc_rtcmc(numtracs,numspec,numradcl,numrxn,
...
c        numradcl   I  number of core model radicals
...
c
         ...
         allocate( srkrtc   (numrxn) )
         allocate( rkrtc    (numrxn) )
c
         allocate( ijschm   (numphot) )
         allocate( zenschm  (numzen) )
         allocate( rjschm   (numzen,numphot) )
c
         allocate( itypsp(numtracs+numspec+numradcl) )
         allocate( itypschm(numtracs+numspec+numradcl) )
         allocate( lblrxn(numrxn) )
         allocate( nrct(numrxn) )
         allocate( nprd(numrxn) )
         allocate( idxrct(numrxn,numrct) )
         allocate( idxprd(numrxn,numprd) )
         allocate( conschm(numtracs+numspec+numradcl) )
         allocate( depvschm(numtracs+numspec+numradcl) )
         allocate( wetschm(numtracs+numspec+numradcl) )
         allocate( spdcoef(numrxn,numprd) )
         allocate( dryrtc(numtracs) )
         allocate( wetrtc(numtracs) )
         allocate( prdcoef(numrxn,numprd) )
         allocate( spnmrt(numtracs+numspec+numradcl) )
         allocate( spnmschm(numtracs+numspec+numradcl) )
...
c
         allocate( idxfix(numtracs+numspec+numradcl) )
c
         return
         end subroutine
c
