...
c
      ...
      enddo
c
c  --- if no BC file provided then we skip to Top Conc section ---
c
      if( .NOT. lbcfil ) goto 222
c
c  --- open the BC ---
c
...
c
     ...
     &                               tpgrd(i,1,k)*prgrd(i,1,k)/1013.
                        ...
                        saconc(i,1,k,imod) = consth(k,j) * convfac
...
c
     ...
     &                              tpgrd(i,noy,k)*prgrd(i,noy,k)/1013.
                       ...
                       saconc(i,noy,k,imod) = connth(k,j) * convfac
...
c
      close(iorbc)
c
c   --- Now deal with the Top Concentrations ---
c
  222 continue
      do imod=1,ntotsp
      ...
      enddo
c
c  --- if no topconc file, then we are done ---
c
      if( .NOT. ltcfil ) goto 9999
c
...
c
 333  continue
      read(iortc,'(A10,F10.0)',END=444) cspec, ctin
      imod = 0
      do i=1,ntotsp
