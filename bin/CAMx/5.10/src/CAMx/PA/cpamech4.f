...
C**   02     01/07  GY   Removed OLE2 reactions
C**   03     08/07  PP|GY  Corrected errors and added HCHOp_eth and HCHOp_ole
C**   
C***********************************************************************
C      
      IMPLICIT NONE
...
C     Ox =O3, NO2, NO3*2, O3P, O1D, HNO3, PAN, HNO4, N2O5*3, ORNIT
     ...
     &       +    R(81)  ! XO2N+NO
     &       +    R(100) ! OH+OH=O
     &       +.39*R(113) ! PNA=0.61*HO2+0.61*NO2+0.39*OH+0.39*NO3
C
C
      nn = nn + 1
      ptname(nn)  = 'OxLoss'
     ...
     &       +    R(103) ! H2O2+O=OH
     &       +2.0*R(104) ! NO3+O=NO2
     &       +    R(105) ! NO3+OH=HO2+NO2
     &       +    R(106) ! NO3+HO2=HNO3
     &       +2.0*R(107) ! NO3+O3=NO2
     &       +2.0*R(108) ! NO3+NO3=2*NO2
C
C
...
C  { 29} HO2+NO2=HNO4
C  { 30} HNO4=HO2+NO2
C  {113} HNO4+hv=0.61{HO2+NO2}+0.39{OH+NO3}
C
      sum =  R(29) - R(30) - 0.61*R(113)  ! HNO4 cycle
      if (sum.GT.0.) then
         cyc_HNO4_p = sum
         cyc_HNO4_l = 0.0
      else
...
C {101} OH+OH=H2O2
C {103} H2O2+O=OH+HO2
      sum =  R(32) + R(33) + R(101) - R(34) - 0.5*R(103)   ! H2O2 cycle
      if (sum.GT.0.) then
         cyc_H2O2_p = 2.*sum
         cyc_H2O2_l = 0.0
      else
...
C...  new OH
     ...
     &                +( 1.000)*r( 97) ! 97 O1D H2 = 1 OH 1 HO2
     &                +( 1.000)*r(103) ! 103 H2O2 O = 1 OH 1 HO2
     &                +( 1.000)*r(110) ! 110 HNO3 = 1 OH 1 NO2
     &                +( 0.390)*r(113) ! 113 PNA = 0.61 HO2 0.61 NO2 0.39 OH
C...  new HO2
      HO2_new =
     ...
     &                +( 1.000)*r( 97) ! 97 O1D H2 = 1 OH 1 HO2
     &                +( 1.000)*r(103) ! 103 H2O2 O = 1 OH 1 HO2
     &                +( 0.610)*r(113) ! 113 PNA = 0.61 HO2 0.61 NO2 0.39 OH
C
...
C...  new XO2
     ...
     &       + 0.075*R(94) ! 94 NO3 ISPD = 0.357 ALD2 0.282 FORM 1.282 PAR
     &       + 1.000*R(96) ! 96 NO2 ISOP = 0.2 ISPD 0.8 NTR XO2 0.8 HO2
     &       + 1.000*R(112)!112 NTR = NO2 XO2
C...  new XO2N
      RO2_new = RO2_new
...
C...  OH reacted
     ...
     &                +( 1.000)*r( 98) ! 98 OH H2 = 1 HO2
     &                +( 1.000)*r( 99) ! 99 OH O = 1 HO2
     &                +( 2.000)*r(100) ! 100 OH OH = 1 O
     &                +( 2.000)*r(101) ! 101 OH OH = 1 H2O2
     &                +( 1.000)*r(105) ! 105 NO3 OH = 1 HO2 1 NO2
C...  HO2 reacted
      HO2_reacted = 
     ...
     &                +( 1.000)*r( 90) ! 90 OH HO2 = 
     &                +( 1.000)*r(102) ! 102 HO2 O = 1 OH
     &                +( 1.000)*r(106) ! 106 NO3 HO2 = 1 HNO3
C
...
C     OH reaction with organic nitrates
     ...
     &        +  1.000*R(110) ! 110 HNO3 = 1 OH 1 NO2
     &        +  1.000*R(112) ! 112 NTR = 1 NO2 1 XO2
C
C
C...  { NO to NO2 by HO2 }
      nn = nn + 1
