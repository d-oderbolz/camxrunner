     ...
     &                   bndtim,bnddate,wrttim,wrtdate,endtim,
     &                   enddate,numprocs)
      use filunit
      use grid
      use chmstry
...
c
      ...
      integer      bnddate, snodate, numprocs
      integer      nlayer, lav, i, j, l, n, ip, ic 
      integer      igrd, ig, kg, kg1, kp, iunit
      integer      icheck, iparm, ism
      real         inptim, emstim, wrttim, endtim, haztim, ozntim
      real         bndtim, snotim, dxmod, dymod, whr, wmn
c
c-----Entry point
c
      icur_unit = 6
...
c
c-----Allocate the arrays for Albedo/Haze/Ozone
c
      call alloc_aho(ngrid, ncol,nrow)
c  
...
c
     ...
     &                   ' mapped to modeled species ',l,spname(l)
        ...
        enddo
        do l = 1,nrad
          if( spavg(lav) .EQ. nmrad(l) ) then
            lradmap(lav) = l
            write(idiag,'(2(A,I5,2X,A))')
     &                   'Average species ',lav,spavg(lav),
     &                   ' mapped to radical species ',l,nmrad(l)
          endif
        enddo
        if( lavmap(lav) .EQ. 0 .AND. lradmap(lav) .EQ. 0 ) then
           write(iout,'(//,a)') 'ERROR in STARTUP:'
...
c             
     ...
     &               'is greater than number of species to model'
        ...
        write(iout,*) 'Model species (NSPEC+NRAD): ',nspec+nrad
        call camxerr()
      endif
c
...
c
          if( lptdepout .AND. .NOT. lhdfout ) then
             call hdrdepsa(iowptdep,ptdepfil,ntotsp,begdate,begtim,
     &                                               enddate,endtim)
          endif
c
...
c
         if( idmech .LT. 3 .OR. idmech .GT. 6 ) then
            write(iout,'(//,a)') 'ERROR in STARTUP:'
...
c
          if ((lchem .or. lwet .or. ldry) .and. ih2o(igrd).eq. 0) then
            write(iout,'(a40,f7.0,i8.5,a,i3)')
