...
c-----------------------------------------------------------------------
c
c    This routine updates all of the fields at the update time-step.
c    It is called when any of the input fields need to be updated
c    (usually every hour). It will call the routine responsible for
c    reading the apopriate file and then pass the information to the
c    slices for MPI.
c
...
c
            if (lipr .OR. lirr) then
               ...
               if (lipr) then
                  do igrd = 1,ngrid
                     call initipr(.TRUE.,igrd,nspec,ncol(igrd),
     &                                     nrow(igrd),nlay(igrd),
     &                                              conc(iptr4d(igrd)))
                  enddo
               endif
