...
c
     ...
     &             avcnc(iptrav(igrd)),ipacl_3d(iptr3d_full(igrd)) )
c
c-----Update PiG contribution
c
      if (ipigflg .NE. 0 .and. LVISPIG) then
         call avepig(igrd,deltat(igrd)/2.0,ncol(igrd),nrow(igrd),
     &               nlay(igrd),nlayav,deltax(1,igrd),deltay(igrd),
     &               mapscl(iptr2d(igrd)),height(iptr3d_full(igrd)),
     &               navspc,nspec,lavmap,
     &               tempk(iptr3d(igrd)),press(iptr3d(igrd)),
     &               avcnc(iptrav(igrd)))
      endif
...
c
     ...
     &                ptavrg(ipsa2d(igrd)),ipacl_3d(iptr3d_full(igrd)))
c
