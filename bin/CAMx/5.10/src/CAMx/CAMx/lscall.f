     ...
     &                  cncrad,avgcnc,avgrad,nirrrxn,rrxn_irr,
     &                  ierr,ldoirr,lpig)
      use chmstry
      use filunit
      use lsbox
...
c
     ...
     2      rrdt,ydd)
      ...
      do i = 1,nrad
        avgrad(i) = MAX(DBLE(bdlrad),y(i))
      enddo
      do i = 1,ngas
        avgcnc(i) = MAX(DBLE(bdnl(i)),y(i+nrad))
      enddo
...
c
      ...
      if (lpig) then
        ...
        do i = 1,ngas
          conc(i) = MAX(DBLE(bdnl(i)),y(i+nrad))
        enddo
      endif
...
c
 900  if (lpig) then 
     ...
     &                            -igrdchm,ichm,jchm
        ...
        y(kNXOY+nrad) = y(kNO3) + 2.0*y(kN2O5)
        y0(kNXOY+nrad) = y0(kNO3) + 2.0*y0(kN2O5)
        do l = 1,ngas
