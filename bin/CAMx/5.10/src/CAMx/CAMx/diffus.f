      subroutine diffus(m1,m2,m3,i0,j0,ia,iz,ja,jz,ibcon,losat,
     ...
     &                  conc,fluxes,depfld,isaptr,tarray2,iproc_id,
...
c$omp&          ldoipts,fcup,fcdn,ipa_idx,n,dtv)
c
...
c
            do l = 1,nspc
              ...
              if( i .GE. ia .AND. i .LE. iz .AND. j .GE. ja
     &                                             .AND. j .LE. jz ) 
     &                    fluxes(l,11) = fluxes(l,11) +  fluxbot(l)*
     &                     dx(j+j0)*dy*depth(i+i0,j+j0,1)/mapscl(i,j)**2
               if( .NOT. losat ) then
                  do ll = 1,navspc
                    if (l .eq. lavmap(ll)) then
                      depfld(i,j,ll) = depfld(i,j,ll) -
     &                                             1.e-2*fluxbot(l)*dtv
                      goto 100
                    endif
                  enddo
...
c$omp&          dfxp,dfxm,dfyp,dfym,fxp,fxm,fyp,fym,isen,idx)
c
c$omp do schedule(dynamic)
c
      do 91 ispc = 1,nspc
           ...
           do isen = 1,nddmsp
             ...
             do j=1,m2   !nrow
               do i = 1,m1   !ncol
                 idx =  isaptr-1 + i + m1*(j-1) + m1*m2*(k-1)+
     &                                               m1*m2*m3*(ioff-1)
                 sns(i,j,isen) = ptconc(idx)/rho(i,j)
                enddo
              enddo
           enddo
         endif
