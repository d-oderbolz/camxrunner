...
c
      data version /'CAMx v5.10, 09-09-18'/
c
c-----Entry point
c
...
c
      if( .NOT. lmpi .OR. iproc_id .GT. 0 ) call avgall( )
c
      call newgrid(1)
c
      call emistrns(1,chtime,chdate,numprocs,iproc_id)
c
c-----Computation for nested grids
c
      if( ngrid .GT. 1 ) call nesting(numprocs,iproc_id)
c
...
c
     ...
     &                     nrow(igrd),ntotsp,ptdryfld(ipsa2d(igrd)),
     &                                         ptwetfld(ipsa2d(igrd)))
                enddo
             endif
c
...
c
           do igrd = 1,ngrid
             ...
             if( lptdepout ) then
                ...
                call zeros(ptdryfld(ipsa2d(igrd)),nodes)
                call zeros(ptwetfld(ipsa2d(igrd)),nodes)
             endif
           enddo
c
