     ...
     &                   H2O,atm,O2,CH4,H2,conc,cncrad,avgcnc,
     &                   avgrad,nirrrxn,rrxn_irr,ldoirr)
      use chmstry
      use filunit
      use lsbox
      implicit none
...
c
      ...
      real      DTIEH
      parameter (LSDIM = MXSPEC + MXSPEC + 1)
...
c
      ...
      real      avgcnc(MXSPEC+1), avgrad(MXSPEC)
c
c --- local variables
c
      integer i, j, l, nstep, istep, istate, iopt
      integer neq(3)
      real    rtol, atol, rstep, scl, t, tout, dtuse
      real    rwork(LRW)
      integer iwork(LIW)
      real    conc0(MXSPEC+1)
      real    cncrad0(MXSPEC+1)
      real    y(LSDIM+5)
      real    yh(LSDIM+5)
      real    yavg(LSDIM)
      real    ydd(LSDIM)
...
c
     ...
     &              nrad+nspfst+1,nrad+ngas)
        do l = nrad+nspfst+1, nrad+ngas
          y(l) = MAX( (y(l) + dtuse*rate(l)), bdnl(l-nrad))
...
c
      do i = 1, ngas
        cncrad(i) = MAX(cncrad(i),bdlrad)
        avgrad(i) = MAX(avgrad(i),bdlrad)
      enddo
      do i = 1, ngas
