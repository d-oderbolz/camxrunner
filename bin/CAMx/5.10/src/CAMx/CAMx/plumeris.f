...
c
      lfirst = .true.
      rise = 0.
      hstk = amax1(hstk,0.001)
      dstk = amax1(dstk,0.01)
      vstk = amax1(vstk,0.01)
c
...
c
  20  continue
      ...
      if (lfirst) then
        if( kstk.gt.1 ) then
           if( hstk.lt.((hght(kstk)+hght(kstk-1))/2.)) kstab = kstk-1
        endif
        if( kstk.eq.nlay ) kstab = kstk-1
      else
        kstab = kstk-1
        bflux = rflux
        ztop = hght(kstk) - hstk
