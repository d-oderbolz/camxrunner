...
c
cbk      scl = conc(kNXOY) / ( cncrad(kNO3) + 2.0*cncrad(kN2O5) )
cbk      cncrad(kNO3)  = scl * cncrad(kNO3)
cbk      cncrad(kN2O5) = scl * cncrad(kN2O5)
c
c======================== DDM Begin =======================
c
cbk      if ( lddm ) then
cbk        do ifam = 1, nfam
cbk          scl = sraddm(ifam,kNO3) + 2.0*sraddm(ifam,kN2O5)
cbk          if (abs(scl).gt.1.e-25) then
cbk            scl = sddm(ifam,kNXOY) / scl
cbk            sraddm(ifam,kNO3)  = scl * sraddm(ifam,kNO3)
cbk            sraddm(ifam,kN2O5) = scl * sraddm(ifam,kN2O5)
cbk          endif
cbk        enddo
cbk      endif
c
...
c
 300  continue
      ...
      avgcnc(kNXOY) = avgrad(kNO3) + 2.0*avgrad(kN2O5)
c
c======================== DDM Begin =======================
c
      if ( lddm ) then
        ...
        enddo
        do ifam = 1, nfam
          sddm(ifam,kNXOY) = sraddm(ifam,kNO3) + 2.0*sraddm(ifam,kN2O5)
        enddo
      endif
c
c======================== DDM End =======================
...
c
 900  continue
     ...
     &                           dtin, clock, dtuse, nstep, niter
      ...
      y1(kNXOY+nrad) = y1(kNO3) + 2.0*y1(kN2O5)
      yinit(kNXOY+nrad) = yinit(kNO3) + 2.0*yinit(kN2O5)
      do l = 1,ngas
