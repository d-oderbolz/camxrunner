      subroutine chemdriv(m1,m2,m3,i0,j0,ia,iz,ja,jz,ibcon,
     &                    igrd,ncol,nrow,nlay,dt,itzon,idfin,fcloud,
...
c$omp&  con_CG,nj,nz,srkrtc,rkrtc,rtcon)
c$omp&  copyin(/ijkgrd/)
c
...
c-----Reset NXOY
c
            con(kNXOY) = crad(kNO3) + 2.0*crad(kN2O5)
c
...
c
            if( aero_flag .GT. 0 ) then
              ...
              avgox(4) = amax1(avgrad(kNO3), bdlrad)
...
c
     ...
     &                 avgrad(koh),avgcnc(ko3),avgrad(kno3),dtchem,
     &                 convfac,irt_cel)
c
c  --- RTCMC chemistry
c
