      subroutine xyadvec(m1,m2,m3,i0,j0,ia,iz,ja,jz,ibcon,
     &                   igrd,xyordr,ncol,nrow,nlay,nspc,nadv,
     &                   deltat,dx,dy,windu,windv,depth,mapscl,conc,
     &                   fluxes,tarray2,isaptr,ipa_cel,iproc_id)
      use filunit
      use chmstry
      use bndary
      use procan
...
c$omp&  private(i,j,k,ispc,l,c1d,v1d,a1d,area,nn,iddm,c1d0,ioff,
...
c
            if (iadvct.eq.2) then
              call hadvbot(m1,dtuse,dx(j+j0),c1d,v1d,a1d,flxarr,
     &                     fpc,fmc,num1d)
...
c
            if ( (lddm .OR. lhddm) .AND. lddmcalc(igrd) ) then
              if ( iadvct .eq. 2 ) then
                call bottddm(m1,dtuse,dx(j+j0),num1d,ntotsp,sen1d,
     &                              nddmsp,c1d0,c1d,fpc,fmc,v1d,a1d)
              elseif ( iadvct .eq. 3 ) then
                do iddm=1,nddmsp
...
c
            flux1 = flxarr(1)   *areamx*dx(j+j0)*mapscl(m_xi1,j)**2
            flux2 = flxarr(m1-1)*areamx*dx(j+j0)*mapscl(m_xi2,j)**2

            if( 1 .GE. ia .AND. 1 .LE. iz .AND. j .GE. ja 
     &                                     .AND. j .LE. jz ) then
...
c
        do 30 i = m_yi1, m_yi2
          ...
          enddo
          do j = 1, m2-1   !nrow-1
            avarea = (dx(j+j0)*depth(i+i0,j+j0,k) + 
...
c
            if (iadvct.eq.2) then
              call hadvbot(m2,dtuse,dy,c1d,v1d,a1d,flxarr,
     &                     fpc,fmc,num1d)
            elseif( iadvct .eq. 3) then
...
c
            if ( (lddm .OR. lhddm) .AND. lddmcalc(igrd) ) then
              if ( iadvct .eq. 2 ) then
                call bottddm(m2,dtuse,dy,num1d,ntotsp,sen1d,nddmsp,
     &                                  c1d0,c1d,fpc,fmc,v1d,a1d)
              elseif ( iadvct .eq. 3 ) then
                do iddm=1,nddmsp
...
c
            flux1 = flxarr(1)   *areamx*dy*mapscl(i,m_yj1)**2
            flux2 = flxarr(m2-1)*areamx*dy*mapscl(i,m_yj2)**2
            if(i .GE. ia .AND. i .LE. iz .AND. 1 .GE. ja 
     &                                     .AND. 1 .LE. jz ) then
               if (flux1.gt.0.0) then
