c*** LOADDM
c
      subroutine loaddm(filflg,numcol,numrow,numlay,nddm,grsens,nddmrad,
     &         gradsen,icl,jcl,kcl,nsen,nspc,nrad,sens,radsen,convfac)
      use tracer
c
c----CAMx v5.10 090918
c
...
c       nddmrad I  number of radical DDM species
c       grsens  R  4-D array of DDM sensitivities
c       gradsen R  4-D array of DDM sensitivities of radical species
...
c       radsen  R  2-D array for this cell and radical species
...
c
      ...
      real    gradsen(numcol,numrow,numlay,nddmrad)
      integer icl
      integer jcl
      integer kcl
      integer nsen
...
c
        do ispc=1,nrad
          do isen=1,nsen
            iddm = (ispc-1)*nddmsp + isen
            gradsen(icl,jcl,kcl,iddm) = radsen(isen,ispc)
          enddo
        enddo
c
...
c
        do ispc=1,nrad
          do isen=1,nsen
            iddm = (ispc-1)*nddmsp + isen
            radsen(isen,ispc) = gradsen(icl,jcl,kcl,iddm)
          enddo
        enddo
c
c  --- done ---
