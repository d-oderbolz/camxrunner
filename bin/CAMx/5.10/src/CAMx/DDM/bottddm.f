      subroutine bottddm(nn,dt,dx,num1d,nspcs,sen,nsen,con0,con,
     &                                          fpc,fmc,vel,area)
      use tracer
c  
c----CAMx v5.10 090918
...
c        num1d               number of cols/rows (first dimension)
...
c
      real sen(MXCELLS,*)
      real con0(nn),con(nn),fpc(nn),fmc(nn),vel(nn),area(nn)
c
      real a0(MXTRSP)
      real a1(MXTRSP)
      real a2(MXTRSP)
...
c-----Test if negative or small concentration was reset in hadvbot to 1.0e-20.
        if (con(i).gt.1.0e-20) then
          do j=1,nsen
            sen(i,j) = sen(i,j) - area(i)*( fp(i,j) - fm(i+1,j) - 
     &                 (fp(i-1,j) - fm(i,j)))
          enddo
        else
          do j=1,nsen
