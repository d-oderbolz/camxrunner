...
c
      ...
      call nodes_pass (nam_M,1,MPI_CHARACTER,itag,numprocs,iproc_id)
      call nodes_pass (nam_O2,1,MPI_CHARACTER,itag,numprocs,iproc_id)
      call nodes_pass (nam_N2,1,MPI_CHARACTER,itag,numprocs,iproc_id)
      call nodes_pass (nam_H2O,1,MPI_CHARACTER,itag,numprocs,iproc_id)
      call nodes_pass (nam_H2,1,MPI_CHARACTER,itag,numprocs,iproc_id)
      call nodes_pass (nam_CH4,1,MPI_CHARACTER,itag,numprocs,iproc_id)
c
...
c
      ...
      call nodes_pass(iptrad,     ngrid,MPI_INTEGER,itag,numprocs,iproc_id)
      ...
      call nodes_pass(ipsa3d_rad, ngrid,MPI_INTEGER,itag,numprocs,iproc_id)
...
c
      if (iproc_id .gt. 0) then
         ...
         call alloc_camxfld_node(nspec,ngrid,navspc,ndepspc,nrad,
     &                                            ncol,nrow,nlay,l3davg)
         call alloc_bndary(ngrid,mmxp,mmyp,nlay,nspec)
         call alloc_ptemiss(nspec,ngrid)
         call alloc_aho_node(ngrid, maxgrds, mmxp,mmyp) 
      endif
      ...
      if( iproc_id .GT. 0 ) then
         if( ipigflg .NE. 0 ) then
            call alloc_pigsty(nrad,nspec,nreactr,ngrid,numprocs,ipigflg)
            if( lsample ) call alloc_pigsty_sample()
         endif
         call alloc_lsbox(nreact,mxspec,mxradcl)
         ...
         if( .NOT. ltrace .AND. .NOT. lddm .AND. .NOT. lhddm ) then
            call alloc_tracer_null(nspec,lirr,ngrid)
            ...
            if( lddm .OR. lhddm ) then
               ...
               call alloc_ddm(ngrid,mmxp,mmyp,nlay,nradddm,
     &                                         nreact,nrateddm,nhddm)
            ...
            if( lptsrc ) call alloc_tracer_ptsrce(nptsrc)
         endif
      endif
