...
c
      real :: dx(*)
      real :: dy
      real :: depths(ncol(ifm),nrow(ifm),nlay(ifm))
c
c-----------------------------------------------------------------------
c    Local variables:
c-----------------------------------------------------------------------
...
c
     ...
     &                      nmesh(ifm),nmesh(ifm),
     &                      dx,dy,depths,nv,k1crs,k2crs,nmshv(1,ifm))
               iptr=iptr+mtp
            enddo
c
...
c
c-----------------------------------------------------------------------
c    BEGIN subroutine node_send_feed_rad:
c-----------------------------------------------------------------------
c
      subroutine node_send_feed_rad(ifm,icm,dx,dy,depths)                             
c
c-----------------------------------------------------------------------
c    Modules used:
c-----------------------------------------------------------------------
...
c
      do nm=1,nmachs
         irecv_req_rad(nm)=0
         if (iget_paths(itype,ifm,nm) .ne. 0) then
            call par_get_noblock(node_buffs(nm)%lbc_rad_recv_buff(1),
     &                  node_buffs(nm)%nrecv,5500+icm+rad_identifier,
     &                           machs(nm),irecv_req_rad(nm)         )
         endif
      enddo
c
c  --- Allocate new temporary buffer if bigger than the old one. ---
c
      if (nbuff_save .eq. 0) then
         ...
         allocate(pbuff_rad(mmzp(ifm)*mmxp(ifm)*mmyp(ifm)*nrad))
         nbuff_save = mmzp(ifm)*mmxp(ifm)*mmyp(ifm)*nrad
      endif
      if (node_buffs(mynum)%nrecv > nbuff_save) then
         if (allocated(pbuff_rad)) deallocate (pbuff_rad)
         allocate (pbuff_rad(node_buffs(mynum)%nrecv))
         nbuff_save=node_buffs(mynum)%nrecv
      endif
c
c  -- Feed back this fine grid's portion of the each coarse grid node --
...
c
            do nv=1,nrad
               ispbeg=iptrad(ifm)+(nv-1)*mmzp(ifm)*mmxp(ifm)*mmyp(ifm)
               call fdbackp(mtp, cncrad(ispbeg),pbuff_rad(1+iptr),
     &                      mmxp(ifm),mmyp(ifm),mmzp(ifm),
     &                      ncol(ifm),nrow(ifm),nlay(ifm),
     &                      i0,j0,
     &                      ifm,icm,i1f-i0,i2f-i0,j1f-j0,j2f-j0,
     ...
     &                      dx,dy,depths,nv,k1crs,k2crs,nmshv(1,ifm))
            ...
            enddo
c
            call par_init_put(node_buffs(nm)%lbc_rad_send_buff(1),
     &                        node_buffs(nm)%nsend            )
            call par_put_int(i1s,1)
            call par_put_int(i2s,1)
            call par_put_int(j1s,1)
            ...
            call par_put_int(nvar,1)
            call par_put_int(iptr,1)
            call par_put_float(pbuff_rad(1),iptr)
            call par_send_noblock(ipaths(5,itype,ifm,nm),
     *                            5500+icm+rad_identifier,
     &                            isend_req_rad(nm) )
         endif
      enddo
c
...
c
c-----------------------------------------------------------------------
c    END subroutine node_send_feed_rad:
c-----------------------------------------------------------------------
c
c-----------------------------------------------------------------------
...
c
      if (nbuff_save .eq. 0) then
         ...
         allocate(pbuff_dry(mmxp(ifm)*mmyp(ifm)*ntotsp))
...
c
      if (nbuff_save .eq. 0) then
         ...
         allocate(pbuff_wet(mmxp(ifm)*mmyp(ifm)*ntotsp))
         nbuff_save = mmxp(ifm)*mmyp(ifm)*ntotsp
      endif
      if (node_buffs(mynum)%nrecv > nbuff_save) then
         if (allocated(pbuff_wet)) deallocate (pbuff_wet)
...
c
      real, dimension(*)        :: dx
      real                      :: dy
      real, dimension(nx,ny,nz) :: depths
c
c-----------------------------------------------------------------------
c    Local variables:
c-----------------------------------------------------------------------
