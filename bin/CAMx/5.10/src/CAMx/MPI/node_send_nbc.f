...
c
     ...
     &                          node_buffs(nm)%nrecv,6000+icm,nm,irecv_req(nm))
      ...
      if (nbuff_save .eq. 0) then
         ...
         allocate( buffnest(mmzp(ifm)*mmxp(ifm)*mmyp(ifm)*nspec) )
...
c
     ...
     &                       i1f,i2f,j1f,j2f,
     &                       offxb,offxe,offyb,offye,
     &                       k1c,k2c,nestratx,nestraty,icm,meshv)
c
c-----------------------------------------------------------------------
c    Modules used:
c-----------------------------------------------------------------------
...
c
     ...
     &                           nm,irecv_req_pt(nm)                         )
      ...
      if (nbuff_save .eq. 0) then
         if (allocated(buffnest)) deallocate (buffnest)
         allocate( buffnest(mmzp(ifm)*mmxp(ifm)*mmyp(ifm)*ntotsp) )
      ...
      do nm=1,nmachs
         if (iget_paths(itype,ifm,nm) .ne. 0) then
            ...
            do nv=1,ntotsp
               ...
               call mknest_buff(ptconc(ispbeg),mtp,buffnest(1+iptr),
     &                          mmxp(icm),mmyp(icm),mmzp(icm),
     &                          mi0(icm),mj0(icm),
     &                          i1c,i2c,j1c,j2c,
     &                          i1f,i2f,j1f,j2f,
