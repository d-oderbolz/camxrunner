...
c    10) root mean square error
      ...
      parameter(mxstn=12,mxhr=744,mxspc=10) ! 744 hours = 1 month
...
c-----Read entire PRED/OBS file
      ...
      read(10,'(i5)') nhr
      read(10,'(i5)') nsite
      if (nhr .gt. mxhr .or. nsite .gt. mxstn .or. nspec .gt. mxspc)then ! BAB
         print*,"Arrays sizes too small:"          ! BAB
         print*,"mxhr  = ",mxhr, " nhr   = ",nhr   ! BAB
         print*,"mxstn = ",mxstn," nsite = ",nsite ! BAB
         print*,"mxspc = ",mxspc," nspec = ",nspec ! BAB
         print*,"Please reset parameter values and re-compile." ! BAB
         stop                   ! BAB
      end if                    ! BAB
      do 5 ns = 1,nsite
...
c-----Calculate stats over all sites and hours
      ...
      nvalida = 0
      nvalidp = 0
      iohrmx = -1 ! BAB: prevent segfault/out-of-bounds error
      do 30 ns = 1,nsite
        omax = -999.
        umax = -999.
        do 40 nh = 1,nhr
 ...
 40     continue
        if (iohrmx .eq. -1) then                          ! BAB
           print*,"No observations above ",cmin," found." ! BAB
           print*,"Increase 'Min Threshold conc' value."  ! BAB
           stop "Error: Min Threshold conc too small."    ! BAB
        endif                                             ! BAB
