c     output course concentration file

      subroutine outcccon

      include 'uamvascii.inc'
c
Ctcm  DIMENSION IFILE(10), NOTE(60)
...
C
      READ(InNum)IFILE,(MNOTE(i),i=1,10),(NOTE(i),i=11,60),
     &           NSEG, NSPECS, IDATE, BEGTIM, JDATE, ENDTIM
      WRITE(OutNum,2100)IFILE,(MNOTE(i),i=1,10),(NOTE(i),i=11,60),
     &                  NSEG,NSPECS,IDATE,BEGTIM,JDATE,ENDTIM
      WRITE (ErrNum,1005) IDATE, BEGTIM, JDATE, ENDTIM
      READ(InNum) ORGX, ORGY, IZONE, UTMX, UTMY, DELTAX, DELTAY,
     $          NX, NY, NZ, NZLOWR, NZUPPR, HTSUR, HTLOW, HTUPP
      WRITE (OutNum,2001) ORGX,ORGY,IZONE,UTMX,UTMY,DELTAX,DELTAY,NX,NY,
     $ NZ,NZLOWR,NZUPPR,HTSUR,HTLOW,HTUPP
      WRITE (ErrNum,2001) ORGX,ORGY,IZONE,UTMX,UTMY,DELTAX,DELTAY,NX,NY,
     $ NZ,NZLOWR,NZUPPR,HTSUR,HTLOW,HTUPP
      READ  (InNum) IX, IY, NXCLL, NYCLL
      WRITE (OutNum,1002) IX,IY,NXCLL,NYCLL
      ...
      READ(InNum) ((MSPEC(M,L),M=1,10),L=1,NSPECS)
   10 IBHR=BEGTIM + 1
      IEHR=ENDTIM
      WRITE (OutNum,1003) ((MSPEC(I,J),I=1,10),J=1,NSPECS)
C*
C    LARGE LOOP FOR 24 HOURS OF AIR QUALITY MEASURMENTS
C
      DO 1000 IH = 1,1000
          READ  (InNum,END=100,err=200) IBGDAT,BEGTIM,IENDAT,ENDTIM
          IF (ABS(BEGTIM-24.) .LT. .01 .AND.
     $        ABS(ENDTIM-0.) .LT. .01) THEN
            BEGTIM = BEGTIM - .5
            ENDTIM = ENDTIM + .5
          ENDIF
          WRITE (OutNum,1005,err=210) IBGDAT, BEGTIM, IENDAT, ENDTIM
...
C
          ...
          DO 400 L = 1,NSPECS
             DO 350 K=1,NZ
                READ (InNum,end=100,err=200)ISEG,(MSPEC(M,L),M=1,10),
     $            ((EMOB(I,J),I=1,NX),J=1,NY)
                WRITE (OutNum,1006,err=210) ISEG,(MSPEC(M,L),M = 1,10)
                WRITE (OutNum,1007,err=210) ((EMOB(I,J),I=1,NX),J=1,NY)
  350        CONTINUE
          WRITE (ErrNum,1002) L
