...
C
C    *** NOTE:  This code is hardwired for the CAMx CB4 mechanism 4 ***
C
...
C**   NO.    DATE   WHO  WHAT
C**   ---    -----  ---  --------------------------------------------
C**   01     09/05  GY   Created from CPAMECH3
C**                      Added OLE2 and extra inorganic reactions
C**                      
C**                         
C***********************************************************************
C      
...
C... Local variable declarations
      ...
      REAL   OHwHC
      REAL   other_OH_prop, prod_hcho_from_isop
...
C...  OH + O3 (accounting for fraction of HO2 recycled via NO)
      PA(nn) =  PA(nn)
     &       - R(12) * ( 2.*(R(32)+R(33))+R(90)+R(86)+R(87)+R(50) )
     &              / ( 2.*(R(32)+R(33))+R(90)+R(86)+R(87)+R(50)+R(28) ) 
C...  O(3P) + VOC
      PA(nn) =  PA(nn)
     &       - ( R(56)+R(60)+R(75)+R(99) )
C...  O3 + VOC
...
cC
cC
cC...  O3 destruction by several pathways treating HOx as a pool
c      nn =  nn + 1
c      ptname(nn)   = 'O3_dest_2'
cC...  O1D + water
c      PA(nn) =  
c     &        - ( 1.000)*R( 11)
cC...  HOx + O3 (accounting for fraction recycled)
c      PA(nn) =  PA(nn)
c     &        - ( ( R( 12)+R( 13) ) * ( 2.*(R(32)+R(33)+R(90))
c     &              + R(86)+R(87) )
c     &            / ( 2.*(R(32)+R(33)+R(90)) + R(86)+R(87)+R(28) ) )
cC...  O(3P) + VOC
c      PA(nn) =  PA(nn)
c     &        - ( R( 56)+R( 60)+R( 75)+R( 99) )
cC...  O3 + VOC
c      PA(nn) =  PA(nn)
c     &        - ( R( 58)+R( 62)+R( 71)+R( 77)+R( 93) )
cC
c      PA(nn) = amin1( PA(nn), (O3_prod - O3_loss) )
C
C
C--------------------End of Ox & O3-----------------------
C
...
C  { 29} HO2+NO2=HNO4
C  { 30} HNO4=HO2+NO2
C  {117} HNO4+hv=0.61{HO2+NO2}+0.39{OH+NO3}
C
      sum =  R(29) - R(30) - 0.61*R(117)  ! HNO4 cycle
      if (sum.GT.0.) then
         cyc_HNO4_p = sum
         cyc_HNO4_l = 0.0
      else
...
C {105} OH+OH=H2O2
C {107} H2O2+O=OH+HO2
      sum =  R(32) + R(33) + R(107) - R(34) - 0.5*R(107)   ! H2O2 cycle
      if (sum.GT.0.) then
         cyc_H2O2_p = 2.*sum
         cyc_H2O2_l = 0.0
      else
...
C...  new OH
     ...
     &                +( 1.000)*r(101) ! 101 O1D H2 = 1 OH 1 HO2
     &                +( 1.000)*r(107) ! 107 H2O2 O = 1 OH 1 HO2
     &                +( 1.000)*r(114) ! 114 HNO3 = 1 OH 1 NO2
     &                +( 0.390)*r(117) ! 117 PNA = 0.61 HO2 0.61 NO2 0.39 OH
C...  new HO2
      HO2_new =
     ...
     &                +( 1.000)*r(101) ! 101 O1D H2 = 1 OH 1 HO2
     &                +( 1.000)*r(107) ! 107 H2O2 O = 1 OH 1 HO2
     &                +( 0.610)*r(117) ! 117 PNA = 0.61 HO2 0.61 NO2 0.39 OH
C
C...  plus HO2 from decomposition of PAN compounds 
...
C...  { New OH from O3 + olefins (only first generation OH) }
     ...
     &       + ( 0.266)*r( 77) ! 77 O3 ISOP = 0.65 ISPD 0.6 FORM 0.2 XO2
     &       + ( 0.268)*r( 93) ! 93 O3 ISPD = 0.114 C2O3 0.15 FORM 0.85 MGLY
     &       + ( 0.100)*r( 99) ! 99 O3 OLE2 = 0.5 ALD2 0.74 FORM 0.22 XO2
C
C
C...  { New HO2 from HCHO }
      nn = nn + 1
...
C...  new XO2
     ...
     &       + 0.220*R(99) ! 99 O3 OLE2 = 0.5 ALD2 0.74 FORM 0.22 XO2
     &       + 0.910*R(59) ! 59 NO3 OLE = 0.91 XO2 1 FORM 0.09 XO2N
     &       + 1.000*R(78) ! 78 NO3 ISOP = 0.2 ISPD 0.8 NTR 1 XO2
     &       + 0.075*R(94) ! 94 NO3 ISPD = 0.357 ALD2 0.282 FORM 1.282 PAR
     &       + 0.910*R(100)! 100 NO3 OLE2 = 0.91 XO2 1 FORM 0.09 XO2N
C...  new XO2N
      RO2_new = RO2_new
...
C...  new TO2
      RO2_new = RO2_new
     &       + 0.560*R(63) ! 63 TOL OH = 0.44 HO2 0.08 XO2 0.36 CRES
     &       + 0.300*R(72) ! 72 OH XYL = 0.7 HO2 0.5 XO2 0.2 CRES
C
...
C...  OH reacted
     ...
     &                +( 1.000)*r( 98) ! 98 OH OLE2 = 1 FORM 1 ALD2 -1 PAR
     &                +( 1.000)*r(102) ! 102 OH H2 = 1 HO2
     &                +( 1.000)*r(103) ! 103 OH O = 1 HO2
     &                +( 2.000)*r(104) ! 104 OH OH = 1 O
     &                +( 2.000)*r(105) ! 105 OH OH = 1 H2O2
     &                +( 1.000)*r(109) ! 109 NO3 OH = 1 HO2 1 NO2
C...  HO2 reacted
      HO2_reacted = 
     ...
     &                +( 1.000)*r( 90) ! 90 OH HO2 = 
     &                +( 1.000)*r(106) ! 106 HO2 O = 1 OH
     &                +( 1.000)*r(110) ! 110 NO3 HO2 = 1 HNO3
C
...
C...  { NOz recycled to NOx }
     ...
     &        +  1.000*R(114) ! 114 HNO3 = 1 OH 1 NO2
     &        +  1.000*R(116) ! 116 NTR = 1 NO2 1 XO2
C
C
C...  { NO to NO2 by HO2 }
      nn = nn + 1
...
C...  { NO to NO2 by RCO3s }
      nn = nn + 1
      ptname(nn)  = 'NOw_RCO3s'
      PA(nn)  =  1.000*R(46)  ! 79 XO2 NO = 1 NO2
C
...
C
      If( nn .GT. MXCPA ) Then
         write(iout,'(//,a)') 'ERROR in CPAMECH4'
         write(iout,'(a)') 
