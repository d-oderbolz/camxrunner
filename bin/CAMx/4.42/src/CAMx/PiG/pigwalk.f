...
c
  ...
  10    kount = kount + 1
     ...
     &                iipig,jjpig,ingrdf,xpigf(n),ypigf(n),pufftop(n),
     &                puffbot(n),windu(iptr3d(igrd)),
...
c  
  ...
  11    kount = kount + 1
        igrd = ingrdb
        call walk1puf(dtcell,igrd,ncol(igrd),nrow(igrd),nlay(igrd),
     &                iipig,jjpig,ingrdb,xpigb(n),ypigb(n),pufftop(n),
     &                puffbot(n),windu(iptr3d(igrd)),
     &                windv(iptr3d(igrd)),height(iptr3d(igrd)),
     &                press(iptr3d(igrd)),tempk(iptr3d(igrd)))
        if (kount.gt.50 .and. dtcell.gt.0.) then
...
c     
     ...
     &     windu(ncol,nrow,nlay),windv(ncol,nrow,nlay)
      integer kpb,kpt,kk
      real wtfac(MXLAYA),deplyr,sumwt,uup,uum,vvp,vvm,dwdz,ztop
c 
c-----Entry point
c
