...
c
     ...
     &     mapscl(ncol,nrow)
      real conpig(MXSPEC+1,MXRECTR),conback(MXSPEC),connew(MXSPEC+1),
     ...
     &        hinox(MXRECTR),ierr1,ierr2,ierr3,aero_flag
      real dt,dy,areamx,pufscl,delt,tpuff,wpuff,ppuff,sumwt,deplyr,
...
c
        ...
        if (noxcnt .ne. nreactr) then
          call ironchem(delt,aero_flag,wpuff,tpuff,ppuff,cwpuff,phpuff,
     &                  connew,cradbck,convfac,ierr1,ierr2,ierr3)
          if (ierr1 .ne. 0) then
            lkill(n) = .true.
            nkill(3) = nkill(3) + 1
...
c
     ...
     &                   conpig(kno2,nr),conpig(ko3,nr),rk(ipigrxn(9)))
          else
            call ironchem(delt,aero_flag,wpuff,tpuff,ppuff,cwpuff,
     &                    phpuff,conpig(1,nr),cradp(1,nr),convfac,ierr1,
     &                    ierr2,ierr3)
            if (ierr1 .ne. 0) then
              lkill(n) = .true.
              nkill(4) = nkill(4) + 1
...
c
        ...
        pufscl = pi*(axisx/2.)*(1.5*sigy(n))
c
        if (lkill(n) .OR. pufscl .GT. areamx .OR. 
     &      fmspig(n) .lt. 0.1 .OR. height(i,j,1) .GT. puffbot(n)) then
          if (pufscl.gt.areamx) nkill(1) = nkill(1) + 1
          if (fmspig(n).lt.0.1) nkill(2) = nkill(2) + 1
          ingrd(n) = 0
          ldump = .true.
          do is = 1,nspec
            dumpmass(is) = 0.
