...
c        05/06/07      Fixed bug in calculating delta concs for Probing
c                      Tools when not performing PM chem step
c
c     Input arguments:
c        igrd                grid index
...
c$omp&  zenith,ldark,delcon,tcell,pcell,cliq,cldph,atm,O2,H2,CH4,sddm,
...
c
            elseif ( idsolv .EQ. IDLSOD ) then
               if (idmech.eq.1) then
                  call lscall(lsrate1,lsjac1,dtchem,water(i,j,k),atm,
     &                                O2,CH4,H2,con,crad,ierr,.false.)
               elseif (idmech.eq.3) then
                  call lscall(lsrate3,lsjac3,dtchem,water(i,j,k),atm,
     &                                O2,CH4,H2,con,crad,ierr,.false.)
               elseif (idmech.eq.4) then
                  call lscall(lsrate4,lsjac4,dtchem,water(i,j,k),atm,
     &                                O2,CH4,H2,con,crad,ierr,.false.)
               elseif (idmech.eq.5) then
                  call lscall(lsrate5,lsjac5,dtchem,water(i,j,k),atm,
     &                                O2,CH4,H2,con,crad,ierr,.false.)
               endif
            endif
c
...
c
     ...
     &                     AMAX1(bdnl(ispc),con(ispc)) * convfac
                 if( aeropt .NE. 'CF' .OR. aero_flag .NE. 2 ) then
...
c
     ...
     &         ( cliq.ge.cwmin .and. tcell.ge.tamin )) then
               call aerochem_aq(MXSPEC,nspec,ngas,aeropt,dtchem,
     &                          water(i,j,k),tcell,pcell,cliq,cldph,
     &                          con,bdnl,convfac,ltrace,delcon,ldoipr,
     &                          ipa_idx)
            elseif( aero_flag.EQ.2 ) then
               if( aeropt.eq.'CF' ) then
                  call aerochem_cf(water(i,j,k),tcell,pcell,cliq,
     &                             cldph,con,convfac,dtchem,
     &                             delcon,ldoipr,ipa_idx)
c
               elseif( aeropt.eq.'CMU' ) then
                  call aerochem_cmu(water(i,j,k),tcell,pcell,cliq,cldph,
     &                              MXSPEC,MXRADCL,con,crad,convfac,
