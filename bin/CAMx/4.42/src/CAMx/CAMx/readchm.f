...
c
      ...
      character*10 nmrad1(14), nmrad3(12), nmrad5(18)
      ...
      integer mchrxn(10), mchphot(10), mchfast(10), mchiessr(10)
...
c
     ...
     &             'BZO       ','BZNO      '/
c
      data mchactv  /  1,  0,  1,  1,  1,  0,  0, 0, 0, 1 /
      data mchgas   / 34, 24, 25, 37, 59,  0,  0, 0, 0, 999 /
      data mchaero  /  0,  0,  0, 18,  0,  0,  0, 0, 0, 999 /
      data mchrad   / 14, 12, 12, 12, 18,  0,  0, 0, 0, 0 /
      data mchiessr /  4,  2,  2,  2,  2,  0,  0, 0, 0, 0 /
      data mchrxn   /110, 91, 96,117,217,  0,  0, 0, 0, 0 /
      data mchphot  / 14, 11, 12, 17, 30,  0,  0, 0, 0, 0 /
      data mchfast  /  4,  4,  4,  4, 13,  0,  0, 0, 0, 0 /
...
c
 ...
 111    continue
     ...
     &                         ' in CHEMPARAM input file: ',aeropt
          if( aeropt .EQ. ' ' ) then
             write(iout,'(2A)') ' When using Mechanism 4 you must',
...
c
      if (lchem) then
        ...
        if (nrad.gt.MXRADCL) then
          write(iout,'(2a)')'Number of radicals is greater than ',
     &                                          'internal dimensions.'
          write(iout,'(a,i3)') 'Number of radicals needed  : ',nrad
          write(iout,'(a,i3)') 'Maximum dimension (MXRADCL): ',MXRADCL
          write(iout,'(2a)')'Increase the parameter MXRADCL ',
...
c
     ...
     &                                          'internal dimensions.'
        write(iout,'(a,i2)') 'Number of species needed  : ',nspec
...
c
      if (nreact.gt.MXRXN) then
        write(iout,'(2a)')'Number of reactions is greater than ',
     &                                          'internal dimensions.'
        write(iout,'(a,i3)') 'Number of reactions needed  : ',nreact
        write(iout,'(a,i3)') 'Maximum dimension (MXRXN)   : ',MXRXN
        write(iout,'(2a)')'Increase the parameter MXRXN ',
     &                                   'in CAMx.PRM and recompile.'
        goto 910
      endif
c
...
c
c-----Check PM species list for consistency with CF and CMU options
c
      if (lchem .and. idmech.NE.10) then
        if (naero.GT.0) then
          if (aeropt.EQ.'CMU') then
c
c-----Check mandatory species for CMU mechanism
c
            if (kso2.eq.nspec+1    .or. kh2o2.eq.nspec+1    .or.
     &          kform.eq.nspec+1   .or. khono.eq.nspec+1    .or.
     &          ko3.eq.nspec+1     .or. koh.eq.nspec+1      .or.
     &          kho2.eq.nspec+1    .or. kno3.eq.nspec+1     .or.
     &          kno.eq.nspec+1     .or. kno2.eq.nspec+1     .or.
     &          kpan.eq.nspec+1    .or. kcg1.eq.nspec+1     .or.
     &          kcg2.eq.nspec+1    .or. kcg3.eq.nspec+1     .or.
     &          kcg4.eq.nspec+1    .or. kcg5.eq.nspec+1     .or.
     &          khno3.eq.nspec+1   .or. knh3.eq.nspec+1     .or.
     &          ksulf.eq.nspec+1   .or. khcl.eq.nspec+1     .or.
     &          ksoa1_1.eq.nspec+1 .or. ksoa2_1.eq.nspec+1  .or.
     &          ksoa3_1.eq.nspec+1 .or. ksoa4_1.eq.nspec+1  .or.
     &          ksoa5_1.eq.nspec+1 .or. kcrst_1.eq.nspec+1  .or.
     &          kpoa_1.eq.nspec+1  .or. kpec_1.eq.nspec+1   .or.
     &          kph2o_1.eq.nspec+1 .or. kpcl_1.eq.nspec+1   .or.
     &          kna_1.eq.nspec+1   .or. kpnh4_1.eq.nspec+1  .or.
     &          kpno3_1.eq.nspec+1 .or. kpso4_1.eq.nspec+1) then
              write(iout,'(/,a)') ' You must have all of the',
     &                   ' following species to use the CMU mechanism:'
              write(iout,'(a)')' SO2,H2O2,FORM,HONO,O3,OH,HO2,NO3,'
              write(iout,'(a)')' NO,NO2,PAN,CG1,CG2,CG3,CG4,CG5,HNO3,'
              write(iout,'(a)')' NH3,SULF,HCL,SOA1,SOA2,SOA3,SOA4,SOA5,'
              write(iout,'(a)')' CRST,POA,PEC,PH2O,PCL,NA,PNH4,PNO3,'
              write(iout,'(a)')' PSO4'
              goto 910
            endif
          elseif (aeropt.EQ.'CF') then
c
c-----Check mandatory species for CF mechanism
c
            if (kso2.eq.nspec+1    .or. kh2o2.eq.nspec+1 .or.
     &          ko3.eq.nspec+1     .or. knxoy.eq.nspec+1 .or.
     &          khno3.eq.nspec+1   .or. knh3.eq.nspec+1  .or.
     &          ksulf.eq.nspec+1   .or. kcg1.eq.nspec+1  .or.
     &          kcg2.eq.nspec+1    .or. kcg3.eq.nspec+1  .or.
     &          kcg4.eq.nspec+1    .or. kcg5.eq.nspec+1  .or.
     &          ksoa1.eq.nspec+1   .or. ksoa2.eq.nspec+1 .or.
     &          ksoa3.eq.nspec+1   .or. ksoa4.eq.nspec+1 .or.
     &          ksoa5.eq.nspec+1   .or. kpso4.eq.nspec+1 .or.
     &          kpno3.eq.nspec+1   .or. kpnh4.eq.nspec+1 .or.
     &          kph2o.eq.nspec+1) then
              write(iout,'(/,a)') ' You must have all of the',
     &                   ' following species to use the CF mechanism:'
              write(iout,'(a)')' SO2,H2O2,O3,NXOY,HNO3,NH3,SULF,'
              write(iout,'(a)')' CG1,CG2,CG3,CG4,CG5,SOA1,SOA2,SOA3,'
              write(iout,'(a)')' SOA4,SOA5,PSO4,PNO3,PNH4,PH2O'
              goto 910
            endif
c
...
c
          if (kcg1.lt.nspec+1 .or. kcg2.lt.nspec+1 .or.
     &        kcg3.lt.nspec+1 .or. kcg4.lt.nspec+1 .or.
     &        kcg5.lt.nspec+1) then
            write(iout,'(/,a)') ' You must have none of the species'
            write(iout,'(a)')   ' CG1,CG2,CG3,CG4,CG5 with non-aerosol'
        ...
        if (ipigflg .NE. 0) then
           if (naero.GT.0) then
             ...
             if (aeropt.EQ.'CF' .AND. ipigflg .EQ. IRONPIG) then
               write(iout,'(/,a)')'IRON PiG cannot be run with M4_CF'
               write(iout,'(/,a)')'Either turn off PiG or change',
     &                            ' to GREASD'
               goto 910
             endif
           endif
...
c
 902  write(iout,'(//,a)') 'ERROR: Reading CHEMPARAM file record:'
      ...
      write(iout,'(a)') 'Check that this is a CAMx4.3 chemparam file'
      goto 910
c
