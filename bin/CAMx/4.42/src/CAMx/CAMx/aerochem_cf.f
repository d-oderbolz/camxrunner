      subroutine aerochem_cf(h2o,tempk,press,cwc,cph,con,
     &                       convfac,dt,delcon,ldoipr,ipa_cel)
c
c----CAMx v4.42 070603
c
...
c
      real cph
      real con(MXSPEC+1), cold(MXSPEC), delcon(4,MXSPEC), delnxoy
      real cwc,cw_kgm3
c
c-----Arrays for SOAP
c
      real soacon(5),cg(5)
      real csatT(5),cpre,mwpre,mwpre0
c
...
c
      ...
      cpre   = con(kpoa)
      mwpre  = mwpre0
      call soap(5,soacon,cg,tempk,convfac,iout,igrdchm,
     &                        ichm,jchm,kchm,.TRUE.,cpre,mwpre,csatT)
...
c
     ...
     &                                +(con(khno3)-cold(khno3))*convfac
          cipr(IPR_AQCHEM,ipa_cel,knxoy)=cipr(IPR_AQCHEM,ipa_cel,knxoy)
     &                                +(con(knxoy)-cold(knxoy))*convfac
          cipr(IPR_AQCHEM,ipa_cel,knh3 )=cipr(IPR_AQCHEM,ipa_cel,knh3 )
     &                                +(con(knh3 )-cold(knh3 ))*convfac
          cipr(IPR_AQCHEM,ipa_cel,kh2o2)=cipr(IPR_AQCHEM,ipa_cel,kh2o2)
     &                                +(con(kh2o2)-cold(kh2o2))*convfac
...
c
     ...
     &                               AMAX1(bdnl(ispc),con(ispc))
         enddo
         delnxoy = delcon(2,knxoy) - delcon(1,knxoy)
         delcon(4,khno3) = delcon(4,khno3) - delnxoy
         delcon(4,knxoy) = delcon(4,knxoy) + delnxoy
      endif
c
c======================== Source Apportion End =======================
c
...
c
c     do is = 1,nspec
c       if (con(is).lt.0.) then
c         write(iout,'(//,A)') ' ERROR in AEROCHEM_CF:'
c         write(iout,'(/,A,i3)') 
c    &              ' Negative concentration for species', is
c         write(iout,'(/,a,4i4)') ' igrd, i, j, k = ',
c    &                igrdchm,ichm,jchm,kchm
c         write(iout,'(/,a,/,4f10.2)')
c    &           '  H2O(ppm)  Temp(K)  Press(mBar) CWC(g/m3)',
c    &                h2o, tempk, press, cwc
c         write(iout,'(//,A)') ' Concentrations are:'
c         write(iout,'(A5,A7,A12,A12)') 
c    &           ' No.','name','C final','C input'
c         do l=1,nspec
c           write(iout,'(i3,2x,a7,1p2e12.3)')
c    &           l, spname(l), con(l), cold(l)
c         enddo
c         call camxerr()
c       endif
c     enddo
