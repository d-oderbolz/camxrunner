...
c
     ...
     &          ro1d(MXLAYA)
      real cnc(MXCOLA,MXROWA),sns(MXCOLA,MXROWA,nspec*nddmsp+1),
     &     rho(MXCOLA,MXROWA)
      real*8 fluxes(nspc,11),fluxbot
      character*20 strz, strxy
      integer nstepv(MXCOLA,MXROWA)
      real dtv,dtmp
c
c-----Entry point
c
...
c========================= Process Analysis End =====================================
c
            call vdiffimp(nlay,dtv,vdep(i,j,ispc),d1d,ro1d,rk1d,c1d,
...
c
            ...
            fluxes(ispc,11) = fluxes(ispc,11) + fluxbot*dx(j)*dy*dtv/
     &                        mapscl(i,j)**2
            do ll = 1,navspc
              if (ispc .eq. lavmap(ll)) then
                depfld(i,j,ll) = depfld(i,j,ll) - 1.e-2*fluxbot*dtv
                goto 100
              endif
            enddo
 100        continue
