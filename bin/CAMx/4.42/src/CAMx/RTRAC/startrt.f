...
c   
c   --- top conditions file ---
c
      action = 'Opening RTRAC Top Concentration file.'
      tcfil = RT_Top_Concentrations
      fname = tcfil
      if( fname .EQ. ' ' ) then
         ltcfil = .FALSE.
         write(iout,9003)
     &                   'RTRAC Top Concentrations file              :',
     &                                                  ' Not supplied'
      else
         ...
         nopen =  nopen + 1
         call getunit(iortc)
         write(iout,9000)'RTRAC Top Concentrations file        (unit):',
     &                                                             iortc
         write(iout,9002) tcfil(:istrln(bcfil))
         ltcfil = .TRUE.
      endif
c
