...
real    :: dx(100),dy(100),xorgn(100),yorgn(100),zz(MXLAYA,100)
...
if (otype == 'sample') then
  ...
  do ngr = 1,ngrds
    ...
    xorgn(ngr) = 1000.*xorgs(ngr)
    yorgn(ngr) = 1000.*yorgs(ngr)
    zz(1,ngr) = 10.
  enddo
else
  ngrds = ngrid
...
!-----Output topography and 3-D layer height fields on the computational domain
   ...
   if (l3davg .and. otype == 'averag') then
      ncount = ncount + 1
      dims(1) = nx(ngr)
      dims(2) = ny(ngr)
      dims(3) = 1
...
call rams_head_fill(ngrds,nx,ny,nz,MXLAYA,runmsg,begdate,begtim, &
