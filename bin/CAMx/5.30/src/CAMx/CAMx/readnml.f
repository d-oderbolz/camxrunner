...
c
     ...
     & TUV_Cloud_Adjust,Root_Output_Name,
...
c
      ...
      integer       numprocs
c      
      integer istrln,i,jj,l,m,n,ibyr,ibmo,ibdy,ieyr,iemo,iedy
      integer ng
...
c
     ...
     &    call alloc_pigsty(nrad,nspec,nreactr,ngrid,
...
c
      if( aeropt .EQ. 'CMU' .AND. ltrace ) then
          write(iout,'(//,a)') 'ERROR in READNML:'
          write(iout,'(/,2A)') ' The source apportionment option is ',
     &                      'not supported with the CMU Aerosol Scheme.'
          write(iout,'(2A)') ' Either turn off source apportionment or ',
     &                            'choose a different Aerosol Scheme.'
          call camxerr()
      endif
c
