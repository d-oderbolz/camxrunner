     ...
     &                  isaptr,tarray2,ipa_xy,ipa_lay,iproc_id)
      use filunit
      use chmstry
      use bndary
      use procan
...
c
      ...
      real         dy
      integer      iproc_id
      integer ispc,j,i1,i2,i,k,numspcs
      real tcpu,dtime
...
c$omp&          fc2,fc3,fluxcls,cnccls,cncafter,icls)
c
...
c
     ...
     &                                         tectyp .NE. RTCMC ) then
               do icls=1,ntrcls
                  do k=1,nlay
                     cnccls(icls,k) = cnccls(icls,k) +
     &                           conc(i,j,k,ispc) * fluxmap(ispc,icls)
                     cncafter(icls,k) = cncafter(icls,k) +
     &                                     c1d(k) * fluxmap(ispc,icls)
                 enddo
                 cnccls(icls,nlay+1) = cnccls(icls,nlay+1) +
     &                              c1d(nlay+1) * fluxmap(ispc,icls)
...
c
     ...
     &                                     .AND. j .LE. jz ) then
                   if (fluxtop.lt.0) then
                     fluxtmp(i,j,1,ispc)  = fluxtmp(i,j,1,ispc) - 
     &                                 fluxtop*dx(j+j0)*dy*deltat
                   else
                     fluxtmp(i,j,2,ispc) = fluxtmp(i,j,2,ispc) - 
     &                                   fluxtop*dx(j+j0)*dy*deltat
                   endif
               endif
            endif
...
c
     ...
     &                                         tectyp .NE. RTCMC ) then
               do icls=1,ntrcls
                  do k=1,nlay
                     fluxcls(icls,k) = fluxcls(icls,k) + 
     &                                  fluxlay(k) * fluxmap(ispc,icls)
                 enddo
               enddo
            endif
c
...
c
          if( ltrace .AND. tectyp .NE. RTRAC .AND.
     &                                      tectyp .NE. RTCMC ) then
             call zfluxsa(m1,m2,m3,ntotsp,ptconc(isaptr),ptrtop,
     &            i,j,rhoscl(i,j),deltat,d1d,fluxcls,cnccls,cncafter)
          endif
c
c======================== Source Apportion End =======================
c
