     ...
     &                  igrd,ncol,nrow,nlay,nspc,ndpspc,deltat,dx,dy,
     &                  idfin,vdep,rkx,rky,rkv,depth,tempk,press,
     &                  uwind,vwind,qvap,cwc,temps,mapscl,sfcz0,
     &                  conc,fluxes,fluxtmp,depfld,isaptr,tarray2,
     &                  iproc_id,ipa_xy,ipa_lay,strz,strxy)
      use filunit
      use chmstry
      use bndary
      use procan
...
c        ndpspc            number of species in depostion array
...
c
      ...
      logical      losat
      logical      ldepfld
      integer      igrd,ncol,nrow,nlay,nspc,ndpspc
      real         deltat
      real         dy
...
c
     ...
     &        fxm,fyp,fym
      ...
      integer n,i,j,k,l,ispc,m,ll,numspcs
c
...
c
      do 601 j =  2,m2-1
        do 501 i = 2,m1-1
          ...
          do k = 1,nlay-1
            dtmp = 0.75*depth(i+i0,j+j0,k)*
     &                         depth(i+i0,j+j0,k+1)/rkv(i,j,k)
            dtv = amin1(dtv,dtmp)
          enddo
          dtv = amax1(dtv,300.)
...
c
          ...
          do n = 1,nstepv(i,j)
            ...
            dzmid = (depth(i+i0,j+j0,2) + depth(i+i0,j+j0,1))/2.
            dpdz  = (press(i,j,2) - press(i,j,1))/dzmid
            press0 = press(i,j,1) - dpdz*depth(i+i0,j+j0,1)/2.
            do k = 1,nlay
...
c
            if (lacm2) then
              call vdiffacm2(nspc,nlay,dtv,temps(i,j),press0,z0,d1d,
     &                      rk1d,qv1d,qc1d,t1d,p1d,w1d,vdry,c1d,fluxbot)
            else
                if ( (lddm .OR. lhddm) .AND. lddmcalc(igrd) ) then
                   call vdiffimp(nspc,nlay,dtv,vdry,d1d,ro1d,rk1d,c1d,
...
c
     ...
     &                     cipr(IPR_BDIF, ipa_idx, l) + fcdn(k,l)
                     else
                        cipr(IPR_DDEP, ipa_idx, l) =
     &                     cipr(IPR_DDEP, ipa_idx, l) + fcdn(k,l)
                     endif
                     cipr(IPR_TDIF, ipa_idx, l) =
     &                     cipr(IPR_TDIF, ipa_idx, l) + fcup(k,l)
                  endif
                enddo
              enddo
            endif
...
c
            do l = 1,numspcs
              do k = 1,nlay
                conc(i,j,k,l) = c1d(k,l)
              enddo
              if( i .GE. ia .AND. i .LE. iz .AND. j .GE. ja
     &                                      .AND. j .LE. jz ) then
                 fluxbot(l) = fluxbot(l)*depth(i+i0,j+j0,1)
                 if( .NOT. losat ) then
                   do k=1,nlay
                      fluxtmp(i,j,k,l) = fluxtmp(i,j,k,l) +
     &                          fluxbot(l)*dx(j+j0)*dy/mapscl(i,j)**2
                   enddo
                   do ll = 1,navspc
                     if( ldepfld .and. l .eq. lavmap(ll) ) then
                       depfld(i,j,ll) = depfld(i,j,ll) - 1.e-2*fluxbot(l)
                       goto 100
                     endif
                   enddo
 100               continue
...
c
     ...
     &                                    .AND. l .LE. ndpspc ) then
                 depfld(i,j,l) = depfld(i,j,l) - 1.e-2*fluxbot(l)
              endif
            enddo
c
      ...
      if( .NOT. losat ) then
         do j = 2,m2-1
           do i = 2,m1-1
             do k=1,nlay
               do l=1,nspc
                  fluxes(l,11) = fluxes(l,11) + fluxtmp(i,j,k,l)
               enddo
             enddo
           enddo
         enddo
