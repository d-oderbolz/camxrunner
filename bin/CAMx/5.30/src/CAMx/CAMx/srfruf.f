...
c 
      ...
      include 'flags.inc'
c
      integer :: m1,m2,m3,iday,iday_m,im,inl,iyear
...
c
c-----Wesely (1989) approach; surface roughness for water is dependent on
c     wind speed
c
...
c
c-----Zhang (2003) approach; roughnes scaled by LAI
c
            elseif (idrydep.eq.2) then
              if (lrdlai) then
                ref_lai = 0.
                do inl = 1,nlu
                  lai_ref_intpl = lai_ref(inl,mbin) + iday/nday(mbin)*
     &                       (lai_ref(inl,mbin+1) - lai_ref(inl,mbin))
                  ref_lai = ref_lai + fsurf(i,j,inl)*lai_ref_intpl
                enddo
                rlai = lai(i,j)/(ref_lai + 1.e-10)
              endif
              im = int(iday/30.5) + 1
              iday_m = iday - int((im - 1)*30.5 + 0.5)
              if (iday_m.eq.0) then
                 im = im - 1
                 iday_m = iday - (im - 1)*30.5
              endif
              lai_f = lai_ref(m,mbin) + iday_m/30.5*
     &                (lai(m,mbin+1) - lai(m,mbin))
              if (lrdlai) then
                lai_f = lai_f*rlai
                if (lai_f.gt.lai_ref(m,15)) then
                  lai_f = lai_ref(m,15)
                else
                  if (lai_f.lt.lai_ref(m,14)) then
                    lai_f = lai_ref(m,14)
                  endif
                endif
              endif
