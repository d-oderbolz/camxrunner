     ...
     &                   ncol,nrow,nlay,nspc,nspcsa,nadv,
...
c
     ...
     &                (mapscl(i,j) + mapscl(i+1,j))
            v1d(i) = windu(i,j,k)
            a1d(i) = mapscl(i,j)*mapscl(i,j)/(dy*depth(i+i0,j+j0,k))
          enddo
          av1d(m1) = dy*depth(m1+i0,j+j0,k)/mapscl(m1,j)
          v1d(m1) = windu(m1,j,k)
          a1d(m1) = mapscl(m1,j)*mapscl(m1,j)/(dy*depth(m1+i0,j+j0,k))
c
