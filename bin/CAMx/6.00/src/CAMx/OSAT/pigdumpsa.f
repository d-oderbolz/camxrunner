...
c
     ...
     &                            kclend,idxspec,idxpig,delconc,saconc)
      use grid
      use pigsty
      use tracer
      implicit none
...
c       idxspec I  the index of the species
...
c
     ...
     &                           fluxmap(idxspec,icls) .GT. 0. ) then
...
c
     ...
     &                      saconc(icell,jcell,kcel,itrc) + 
     &                            delconc(kcel) * fluxmap(idxspec,icls)
c
...
c
                       ...
                       if( tectyp .EQ. GOAT ) then
                           saconc(icell,jcell,kcel,itrc) = 
     &                          saconc(icell,jcell,kcel,itrc) + 
     &                            delconc(kcel) * trspmap(idxspec,icls)
c
