     ...
     &                    pws,pwg,cph,densfac,idfin,conc,solmass,
     &                    vegmass,fsurf)
      use bndary
      use chmstry
      use tracer
...
c
      integer ncol,nrow,nlay,nrtsp,igrid,j,i,kbot,ktop,
     &        k,ncnt,l,n
      integer idfin(m1,m2)
      real deltax(nrow)
     ...
     &                             cwc,pwr,pws,pwg,cph
      ...
      real mapscl(m1,m2)
      real fsurf(m1,m2,NLUW89)
      real rd,rhoh2o,deltat,deltay,densfac,cellvol,rainvol,rhoair,
     &     delc,delm,convfac,cmin,hlaw,gscav,ascav,totc,totw,ceq,
     &     psize,cwat,pwat,rconst,dtfall,drpvel,delc0,depmas
      real area,solmass(m1,m2,nrtsp),vegmass(m1,m2,nrtsp)
...
c
     ...
     &                        depth(i,j,kbot)/mapscl(i,j)**2
            ...
            enddo
c
c-----Otherwise rain reaches the ground, increment surface mass array
c
          elseif (lsrfmod) then
            ...
            do l = 1,nrtrac
              ...
              do n = 1,NLUW89
                iwater = 7
                if (n.ne.iwater) then
                  solmass(i,j,l) = solmass(i,j,l) +
     &                                    fsoil(n)*fsurf(i,j,n)*depmas
                  vegmass(i,j,l) = vegmass(i,j,l) +
     &                             (1. - fsoil(n))*fsurf(i,j,n)*depmas
                endif
              enddo
            enddo
          endif
