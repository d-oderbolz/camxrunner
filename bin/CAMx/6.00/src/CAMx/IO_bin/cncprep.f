...
c
     ...
     &        nx,ny,nz,iproj,idum,i,n,l,lic,nlayer
...
c
      read(iunit) plon,plat,iutm,orgx,orgy,dx,dy,nx,ny,nz,
     &                     iproj,idum,t1,t2
      if (.NOT.llatlon) then
        orgx = orgx/1000.
        orgy = orgy/1000.
...
c
      read(iunit) 
      read(iunit) ((icspec(n,l),n=1,10),l=1,nicspc)
c
...
c
      ...
      do l = 1,navspc
        if(lavmap(l) .GT. 0 ) read(spname(lavmap(l)),'(10a1)') 
     &                                        (icspec(n,l),n=1,10)
      enddo
      if( l3davg(1) ) then
        nlayer = nlay(1)
