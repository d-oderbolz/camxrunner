     ...
     &                    tsurf,press,water,windu,windv,cldtrns,
     &                    cwc,pwr,pws,pwg,cph,cellat,cellon,topo,sfcz0,
     &                    vdep,conc,pigdump,pgmserr,fluxes,depfld,
     &                    ipsa2d,ipsa3d,ipsadep,ipa_cel,iproc_id)
      use camxcom
      use filunit
      use chmstry
      use o3colmap
...
c
      logical ldarkp,ldump,lvdmp(MXPIG),lkill(MXPIG),lstage3,lnogas
      logical lnegno,lchemp
      ...
      real    dpfl(2*MXSPEC)
      real    delconc(MXLAYER),avgcnc(MXSPEC+1)
c
      integer n,nn,nj,nz,i,j,k,kk,kkk,l,ll,is,ierr,
...
c$omp&          noxcnt,hinox,ij,iozon,trn,nn,alb,hght,
c$omp&          ctrns,zenith,ldarkp,lstage3,test,
c$omp&          ierr1,ierr2,ierr3,rtcon,avgcnc,nj,
...
c$omp&          dtchem,nz,ierr,lnogas,totgas,lnegno)
c
        ...
        do is = nrad+1,nspec
          do nr = 1,nreactr
            ...
            if (is.le.ngas) then
              ...
              totgas(nr) = totgas(nr) + abs(conpig(is,nr))
              if (ipigflg.EQ.GRESPIG .and. is.eq.kNO .and.
     &            conpig(is,nr).lt.0.) lnegno = .true.
            endif
...
c
          ...
          enddo
          lnegno = .false.
          do 31 l = nrad+1,nspec
            do nr = 1,nreactr
              conpig(l,nr) = puffmass(l,nr,n)/(volpuff/float(nreactr))
              if (l.le.ngas) then
                conpig(l,nr) = conpig(l,nr)/convfac
                totgas(nr) = totgas(nr) + abs(conpig(l,nr))
                if (ipigflg.EQ.GRESPIG .and. l.eq.kNO .and.
     &              conpig(l,nr).lt.0.) lnegno = .true.
              endif
...
c
          if( lptdepout ) then
             call pigwetsa(m1,m2,nrad,nspec,notimespc,i,j,n,dpfl,
     &                                         ptwetfld(ipsadep) )
          endif
c
c======================== Source Apportion Begin =======================
...
c
        ...
        lnogas = .false.
        do nr = 1,nreactr
          if (totgas(nr).lt.0.001) lnogas = .true.
          if (conpig(kNO2,nr).le.0.) then
            test = 999.0
          elseif (idmech.eq.5 .OR. idmech.eq.8) then
     ...
     &          rk(28)*conpig(kNO2,nr)
          elseif (idmech.eq.1 .OR. idmech.eq.7) then
            test = (  rk(52)*conpig(kSO2,nr) +
     &          rk(123)*conpig(kCO,nr)   + rk(124)*CH4              +
     &          rk(126)*conpig(kMEOH,nr) + rk(96)*conpig(kFORM,nr)  +
     &          rk(106)*conpig(kALD2,nr) + rk(110)*conpig(kALDX,nr) +
     &          rk(125)*conpig(kETHA,nr) + rk(127)*conpig(kETOH,nr) +
     &          rk(132)*conpig(kPAR,nr)  + rk(142)*conpig(kOLE,nr)  +
     &          rk(138)*conpig(kETH,nr)  + rk(146)*conpig(kIOLE,nr) +
     &          rk(177)*conpig(kTOL,nr)  + rk(187)*conpig(kCRES,nr) +
     &          rk(204)*conpig(kOPEN,nr) + rk(182)*conpig(kXYL,nr)  +
     &          rk(121)*conpig(kMGLY,nr) + rk(149)*conpig(kISOP,nr) +
     &          rk(157)*conpig(kISPD,nr) + rk(169)*conpig(kTERP,nr) +
     &          rk(131)*conpig(kPRPA,nr) + rk(172)*conpig(kBENZ,nr) +
     &          rk(136)*conpig(kETHY,nr) + rk(130)*conpig(kACET,nr) +
     &          rk(200)*conpig(kXOPN,nr) ) /
     &          rk(45)*conpig(kNO2,nr)
     ...
     &          rk(132)*conpig(kPAR,nr)  + rk(142)*conpig(kOLE,nr)  +
     &          rk(138)*conpig(kETH,nr)  + rk(146)*conpig(kIOLE,nr) +
     &          rk(180)*conpig(kTOL,nr)  + rk(190)*conpig(kCRES,nr) +
     &          rk(202)*conpig(kOPEN,nr) + rk(185)*conpig(kXYL,nr)  +
     &          rk(121)*conpig(kMGLY,nr) + rk(149)*conpig(kISOP,nr) +
     &          rk(158)*conpig(kISPD,nr) + rk(162)*conpig(kISPX,nr) +
     &          rk(163)*conpig(kHPLD,nr) + rk(165)*conpig(kEPOX,nr) +
     &          rk(170)*conpig(kINTR,nr) + rk(172)*conpig(kTERP,nr) +
     &          rk(131)*conpig(kPRPA,nr) + rk(175)*conpig(kBENZ,nr) +
     &          rk(136)*conpig(kETHY,nr) + rk(130)*conpig(kACET,nr) +
     &          rk(198)*conpig(kXOPN,nr) + rk(205)*conpig(kCAT1,nr)) /
     &          rk(45)*conpig(kNO2,nr)
          else
            write(iout,'(/,a)')
     &          'In PIGDRIVE, selected mechanism not implemented'
                call flush(iout)
                call camxerr()
          endif
          if (test.gt.0.1 .and. conpig(kNO2,nr).lt.0.1) lstage3 = .true.
        enddo
        if (test.eq.999. .OR. lnogas .OR. lnegno .OR.
     &      (lstage3 .AND. ipigflg.EQ.GRESPIG)) then
          lkill(n) = .true.
          nkill(5) = nkill(5) + 1
          goto 100
        endif
...
c-----Calculate the rate constant for heterogeneous hydrolysis of N2O5
c
          call khetero(tpuff,wpuff,conpig(1,nr))
c
