c-----------------------------------------------------------------------
c this is the double precision version of lsode from netlib
c changes:
c  - changed the following subroutine names by appending a "d"
c    to make them differ from the single precision version:
c    cfode ewset intdy prepj solsy stode vnorm xerrwv xsetf xsetun srcom
c    likewise changed common block /eh0001/ to /ehd001/
c    likewise changed common block /ls0001/ to /lsd001/
c  - calculate integrated reaction rates (rrdt)
c    pass nrr and rrxn to/from subroutine f
cgy
c-----------------------------------------------------------------------
      subroutine dlsode (f, neq, y, t, tout, itol, rtol, atol, itask,
     1            istate, iopt, rwork, lrw, iwork, liw, jac, mf,
     2            lrr, nrr, rrxn, rrdt, ydd)
      external f, jac
      ...
      logical lrr
      integer nrr
      real*8 trr, delt, rrdt(nrr), rrxn(nrr), ydd(1)
c-----------------------------------------------------------------------
...
c          if only the derivative dy/dt is needed, use intdyd instead.
c
...
c (the routines xsetund and xsetfd are designed to conform to the
c slatec error handling package.)
c
c     form of call                  function
c   call xsetund(lun)          set the logical unit number, lun, for
c                             output of messages from lsode, if
c                             the default is not desired.
c                             the default value of lun is 6.
c
c   call xsetfd(mflag)         set a flag to control the printing of
c                             messages by lsode.
...
c
c   call srcomd(rsav,isav,job) saves and restores the contents of
...
c                                srcomd is useful if one is
c                             interrupting a run and restarting
c                             later, or alternating between two or
c                             more problems solved with lsode.
c
c   call intdyd(,,,,,)         provide derivatives of y, of various
c        (see below)          orders, at a specified point t, if
c                             desired.  it may be called only after
c                             a successful return from lsode.
c
c the detailed instructions for using intdyd are as follows.
c the form of the call is..
c
c   call intdyd (t, k, rwork(21), nyh, dky, iflag)
c
...
c             derivative dy/dt is always available with intdyd.
...
c         /lsd001/  of length  257  (218 double precision words
c                         followed by 39 integer words),
c         /ehd001/  of length  2 (integer words).
c
...
c blocks, use subroutine srcomd (see part ii above).
c
c-----------------------------------------------------------------------
...
c     subroutine ewsetd (neq, itol, rtol, atol, ycur, ewt)
c where neq, itol, rtol, and atol are as in the lsode call sequence,
c ycur contains the current dependent variable vector, and
c ewt is the array of weights set by ewsetd.
c
c if the user supplies this subroutine, it must return in ewt(i)
c (i = 1,...,neq) a positive quantity suitable for comparing errors
c in y(i) to.  the ewt array returned by ewsetd is passed to the
c vnormd routine (see below), and also used by lsode in the computation
c of the optional output imxer, the diagonal jacobian approximation,
c and the increments for difference quotient jacobians.
c
c in the user-supplied version of ewsetd, it may be desirable to use
c the current values of derivatives of y.  derivatives up to order nq
c are available from the history array yh, described above under
c optional outputs.  in ewsetd, yh is identical to the ycur array,
c extended to nq + 1 columns with a column length of nyh and scale
c factors of h**j/factorial(j).  on the first call for the problem,
c given by nst = 0, nq is 1 and h is temporarily set to 1.0.
c the quantities nq, nyh, h, and nst can be obtained by including
c in ewsetd the statements..
c     double precision h, rls
c     common /lsd001/ rls(218),ils(39)
c     nq = ils(35)
c     nyh = ils(14)
c     nst = ils(36)
c     h = rls(212)
...
c vnormd is called with n = neq and with w(i) = 1.0/ewt(i), where
c ewt is as set by subroutine ewsetd.
c
c if the user supplies this function, it should return a non-negative
c value of vnormd suitable for use in the error control in lsode.
c none of the arguments should be altered by vnormd.
c for example, a user-supplied vnormd routine might..
...
c  intdyd    computes an interpolated value of the y vector at t = tout.
c  stoded    is the core integrator, which does one step of the
c           integration and the associated error control.
c  cfoded    sets all method coefficients and test constants.
c  prepjd    computes and preprocesses the jacobian matrix j = df/dy
c           and the newton iteration matrix p = i - h*l0*j.
c  solsyd    manages solution of linear system in chord iteration.
c  ewsetd    sets the error weight vector ewt before each step.
c  vnormd    computes the weighted r.m.s. norm of a vector.
c  srcomd    is a user-callable routine to save and restore
...
c  xerrwd, xsetund, and xsetfd   handle the printing of all error
c           messages and warnings.  xerrwd is machine-dependent.
c note..  vnormd, idamax, ddot, and d1mach are function routines.
...
c if istate = 3, set flag to signal parameter changes to stoded. --------
      jstart = -1
...
c load and invert the ewt array.  (h is temporarily set to 1.0.) -------
      nq = 1
      h = 1.0d0
      call ewsetd (n, itol, rtol, atol, rwork(lyh), rwork(lewt))
      do 120 i = 1,n
...
c-----------------------------------------------------------------------
 ...
 160  tol = dmax1(tol,100.0d0*uround)
      ...
      sum = vnormd (n, rwork(lf0), rwork(lewt))
...
c-----------------------------------------------------------------------
 ...
 210  if ((tn - tout)*h .lt. 0.0d0) go to 250
      call intdyd (tout, 0, rwork(lyh), nyh, y, iflag)
...
c the call to the one-step core integrator stoded.
c
...
c-----------------------------------------------------------------------
 ...
 270  tolsf = uround*vnormd (n, rwork(lyh), rwork(lewt))
 ...
 280  if ((tn + h) .ne. tn) go to 290
      ...
      call xerrwd(50hlsode--  warning..internal t (=r1) and h (=r2) are,
     1   50, 101, 0, 0, 0, 0, 0, 0.0d0, 0.0d0)
      call xerrwd(
     1  60h      such that in the machine, t + h = t on the next step  ,
     1   60, 101, 0, 0, 0, 0, 0, 0.0d0, 0.0d0)
      call xerrwd(50h      (h = step size). solver will continue anyway,
     1   50, 101, 0, 0, 0, 0, 2, tn, h)
      if (nhnil .lt. mxhnil) go to 290
      call xerrwd(50hlsode--  above warning has been issued i1 times.  ,
     1   50, 102, 0, 0, 0, 0, 0, 0.0d0, 0.0d0)
      call xerrwd(50h      it will not be issued again for this problem,
     1   50, 102, 0, 1, mxhnil, 0, 0, 0.0d0, 0.0d0)
 290  continue
c-----------------------------------------------------------------------
c     call stoded(neq,y,yh,nyh,yh,ewt,savf,acor,wm,iwm,f,jac,prepjd,solsyd)
c-----------------------------------------------------------------------
      call stoded (neq, y, rwork(lyh), nyh, rwork(lyh), rwork(lewt),
     1   rwork(lsavf), rwork(lacor), rwork(lwm), iwork(liwm),
     2   f, jac, prepjd, solsyd,
     3   nrr, rrxn)
      kgo = 1 - kflag
...
c-----------------------------------------------------------------------
 300  init = 1
cgy - calculate integrated reaction rates (rrdt)
      if (lrr) then
        delt = min(tout,tn) - trr
        trr = tn
        call f (neq, t, y, ydd, nrr, rrxn)
        do i = 1,nrr
          rrdt(i) = rrdt(i) + rrxn(i)*delt
        enddo
cdebug        write(*,'(1P5E10.3)') trr, tn, h, delt, trr+delt
cdebug        write(*,'(1P7E10.3)') (rrdt(i),i = 1,nrr)
      endif
cgy
...
c the maximum number of steps was taken before reaching tout. ----------
 500  call xerrwd(50hlsode--  at current t (=r1), mxstep (=i1) steps   ,
     1   50, 201, 0, 0, 0, 0, 0, 0.0d0, 0.0d0)
      call xerrwd(50h      taken on this call before reaching tout     ,
     1   50, 201, 0, 1, mxstep, 0, 1, tn, 0.0d0)
      istate = -1
      go to 580
c ewt(i) .le. 0.0 for some i (not at start of problem). ----------------
 510  ewti = rwork(lewt+i-1)
      call xerrwd(50hlsode--  at t (=r1), ewt(i1) has become r2 .le. 0.,
     1   50, 202, 0, 1, i, 0, 2, tn, ewti)
      istate = -6
      go to 580
c too much accuracy requested for machine precision. -------------------
 520  call xerrwd(50hlsode--  at t (=r1), too much accuracy requested  ,
     1   50, 203, 0, 0, 0, 0, 0, 0.0d0, 0.0d0)
      call xerrwd(50h      for precision of machine..  see tolsf (=r2) ,
     1   50, 203, 0, 0, 0, 0, 2, tn, tolsf)
      rwork(14) = tolsf
      istate = -2
      go to 580
c kflag = -1.  error test failed repeatedly or with abs(h) = hmin. -----
 530  call xerrwd(50hlsode--  at t(=r1) and step size h(=r2), the error,
     1   50, 204, 0, 0, 0, 0, 0, 0.0d0, 0.0d0)
      call xerrwd(50h      test failed repeatedly or with abs(h) = hmin,
     1   50, 204, 0, 0, 0, 0, 2, tn, h)
      istate = -4
      go to 560
c kflag = -2.  convergence failed repeatedly or with abs(h) = hmin. ----
 540  call xerrwd(50hlsode--  at t (=r1) and step size h (=r2), the    ,
     1   50, 205, 0, 0, 0, 0, 0, 0.0d0, 0.0d0)
      call xerrwd(50h      corrector convergence failed repeatedly     ,
     1   50, 205, 0, 0, 0, 0, 0, 0.0d0, 0.0d0)
      call xerrwd(30h      or with abs(h) = hmin   ,
...
c the run is halted.
c-----------------------------------------------------------------------
 601  call xerrwd(30hlsode--  istate (=i1) illegal ,
     1   30, 1, 0, 1, istate, 0, 0, 0.0d0, 0.0d0)
      go to 700
 602  call xerrwd(30hlsode--  itask (=i1) illegal  ,
     1   30, 2, 0, 1, itask, 0, 0, 0.0d0, 0.0d0)
      go to 700
 603  call xerrwd(50hlsode--  istate .gt. 1 but lsode not initialized  ,
     1   50, 3, 0, 0, 0, 0, 0, 0.0d0, 0.0d0)
      go to 700
 604  call xerrwd(30hlsode--  neq (=i1) .lt. 1     ,
     1   30, 4, 0, 1, neq(1), 0, 0, 0.0d0, 0.0d0)
      go to 700
 605  call xerrwd(50hlsode--  istate = 3 and neq increased (i1 to i2)  ,
     1   50, 5, 0, 2, n, neq(1), 0, 0.0d0, 0.0d0)
      go to 700
 606  call xerrwd(30hlsode--  itol (=i1) illegal   ,
     1   30, 6, 0, 1, itol, 0, 0, 0.0d0, 0.0d0)
      go to 700
 607  call xerrwd(30hlsode--  iopt (=i1) illegal   ,
     1   30, 7, 0, 1, iopt, 0, 0, 0.0d0, 0.0d0)
      go to 700
 608  call xerrwd(30hlsode--  mf (=i1) illegal     ,
     1   30, 8, 0, 1, mf, 0, 0, 0.0d0, 0.0d0)
      go to 700
 609  call xerrwd(50hlsode--  ml (=i1) illegal.. .lt.0 or .ge.neq (=i2),
     1   50, 9, 0, 2, ml, neq(1), 0, 0.0d0, 0.0d0)
      go to 700
 610  call xerrwd(50hlsode--  mu (=i1) illegal.. .lt.0 or .ge.neq (=i2),
     1   50, 10, 0, 2, mu, neq(1), 0, 0.0d0, 0.0d0)
      go to 700
 611  call xerrwd(30hlsode--  maxord (=i1) .lt. 0  ,
     1   30, 11, 0, 1, maxord, 0, 0, 0.0d0, 0.0d0)
      go to 700
 612  call xerrwd(30hlsode--  mxstep (=i1) .lt. 0  ,
     1   30, 12, 0, 1, mxstep, 0, 0, 0.0d0, 0.0d0)
      go to 700
 613  call xerrwd(30hlsode--  mxhnil (=i1) .lt. 0  ,
     1   30, 13, 0, 1, mxhnil, 0, 0, 0.0d0, 0.0d0)
      go to 700
 614  call xerrwd(40hlsode--  tout (=r1) behind t (=r2)      ,
     1   40, 14, 0, 0, 0, 0, 2, tout, t)
      call xerrwd(50h      integration direction is given by h0 (=r1)  ,
     1   50, 14, 0, 0, 0, 0, 1, h0, 0.0d0)
      go to 700
 615  call xerrwd(30hlsode--  hmax (=r1) .lt. 0.0  ,
     1   30, 15, 0, 0, 0, 0, 1, hmax, 0.0d0)
      go to 700
 616  call xerrwd(30hlsode--  hmin (=r1) .lt. 0.0  ,
 ...
 618  call xerrwd(
     ...
     1   60, 18, 0, 2, leniw, liw, 0, 0.0d0, 0.0d0)
      go to 700
 619  call xerrwd(40hlsode--  rtol(i1) is r1 .lt. 0.0        ,
     1   40, 19, 0, 1, i, 0, 1, rtoli, 0.0d0)
      go to 700
 620  call xerrwd(40hlsode--  atol(i1) is r1 .lt. 0.0        ,
     1   40, 20, 0, 1, i, 0, 1, atoli, 0.0d0)
      go to 700
 621  ewti = rwork(lewt+i-1)
      call xerrwd(40hlsode--  ewt(i1) is r1 .le. 0.0         ,
 ...
 625  call xerrwd(
     ...
     1   60, 25, 0, 0, 0, 0, 2, tcrit, tout)
      go to 700
 626  call xerrwd(50hlsode--  at start of problem, too much accuracy   ,
     ...
     1   60, 26, 0, 0, 0, 0, 1, tolsf, 0.0d0)
      rwork(14) = tolsf
      go to 700
 627  call xerrwd(50hlsode--  trouble from intdy. itask = i1, tout = r1,
...
c
 700  if (illin .eq. 5) go to 710
      ...
      istate = -3
      return
 710  call xerrwd(50hlsode--  repeated occurrences of illegal input    ,
     1   50, 302, 0, 0, 0, 0, 0, 0.0d0, 0.0d0)
c
 800  call xerrwd(50hlsode--  run aborted.. apparent infinite loop     ,
     1   50, 303, 2, 0, 0, 0, 0, 0.0d0, 0.0d0)
      return
c----------------------- end of subroutine lsode -----------------------
      end
      subroutine cfoded (meth, elco, tesco)
clll. optimize
      integer meth
...
c-----------------------------------------------------------------------
c cfoded is called by the integrator routine to set coefficients
c needed there.  the coefficients for the current method, as
c given by the value of meth, are set for all orders and saved.
c the maximum order assumed here is 12 if meth = 1 and 5 if meth = 2.
c (a smaller value of the maximum order is also allowed.)
c cfoded is called once at the beginning of the problem,
...
c store coefficients in elco and tesco. --------------------------------
 ...
 230    continue
      return
c----------------------- end of subroutine cfoded -----------------------
      end
      subroutine ewsetd (n, itol, rtol, atol, ycur, ewt)
clll. optimize
c-----------------------------------------------------------------------
...
c
 ...
 45     ewt(i) = rtol(i)*dabs(ycur(i)) + atol(i)
      return
c----------------------- end of subroutine ewsetd -----------------------
      end
      subroutine intdyd (t, k, yh, nyh, dky, iflag)
clll. optimize
      integer k, nyh, iflag
      integer iownd, iowns,
...
c$omp threadprivate(/lsd001/)
c-----------------------------------------------------------------------
c intdyd computes interpolated values of the k-th derivative of the
...
c
 ...
 60     dky(i) = r*dky(i)
      return
c
 80   call xerrwd(30hintdyd--  k (=i1) illegal      ,
     1   30, 51, 0, 1, k, 0, 0, 0.0d0, 0.0d0)
      iflag = -1
      return
 90   call xerrwd(30hintdyd--  t (=r1) illegal      ,
     ...
     1   60, 52, 0, 0, 0, 0, 2, tp, tn)
      iflag = -2
      return
c----------------------- end of subroutine intdyd -----------------------
      end
      subroutine prepjd (neq, y, yh, nyh, ewt, ftem, savf, wm, iwm,
     1   f, jac,
     2   nrr, rrxn)
clll. optimize
      external f, jac
...
c$omp threadprivate(/lsd001/)
c-----------------------------------------------------------------------
c prepjd is called by stoded to compute and process the matrix
...
c with prepjd uses the following..
c y     = array containing predicted values on entry.
c ftem  = work array of length n (acor in stoded).
...
c do lu decomposition of p. --------------------------------------------
      ...
      if (ier .ne. 0) ierpj = 1
      return
c----------------------- end of subroutine prepjd -----------------------
      end
      subroutine solsyd (wm, iwm, x, tem)
clll. optimize
      integer iwm
      integer iownd, iowns,
     1   icf, ierpj, iersl, jcur, jstart, kflag, l, meth, miter,
...
c communication with solsyd uses the following variables..
...
c
 400  ml = iwm(1)
      ...
      return
c----------------------- end of subroutine solsyd -----------------------
      end
      subroutine stoded (neq, y, yh, nyh, yh1, ewt, savf, acor,
...
clll. optimize
     ...
     1   r, rh, rhdn, rhsm, rhup, told, vnormd
      dimension neq(1), y(1), yh(nyh,1), yh1(1), ewt(1), savf(1),
     1   acor(1), wm(1), iwm(1)
      integer nrr
      real*8 rrxn(nrr)
      common /lsd001/ conit, crate, el(13), elco(13,12),
...
c$omp threadprivate(/lsd001/)
c-----------------------------------------------------------------------
c stoded performs one step of the integration of an initial value
c problem for a system of ordinary differential equations.
c note.. stoded is independent of the value of the iteration method
c indicator miter, when this is .ne. 0, and hence is independent
c of the type of chord method used, or the jacobian structure.
c communication with stoded is done with the following variables..
c
...
c if the caller has changed meth, cfoded is called to reset
...
c-----------------------------------------------------------------------
 ...
 125    el(i) = elco(i,nq)
      ...
      ddn = vnormd (n, savf, ewt)/tesco(1,l)
      exdn = 1.0d0/dfloat(l)
      ...
      go to 175
c-----------------------------------------------------------------------
c cfoded is called to get all the integration coefficients for the
c current meth.  then the el vector and related constants are reset
c whenever the order nq is changed, or at the start of the problem.
c-----------------------------------------------------------------------
 140  call cfoded (meth, elco, tesco)
 150  do 155 i = 1,l
 155    el(i) = elco(i,nq)
      nqnyh = nq*nyh
      rc = rc*el(1)/el0
...
c-----------------------------------------------------------------------
 450  jcur = 0
      ...
      if (m .gt. 0) dsm = vnormd (n, acor, ewt)/tesco(2,nq)
...
c-----------------------------------------------------------------------
 ...
 530    savf(i) = acor(i) - yh(i,lmax)
      dup = vnormd (n, savf, ewt)/tesco(3,nq)
 ...
 540  exsm = 1.0d0/dfloat(l)
      ...
      ddn = vnormd (n, yh(1,l), ewt)/tesco(1,nq)
...
c-----------------------------------------------------------------------
 ...
 720  hold = h
      jstart = 1
      return
c----------------------- end of subroutine stoded -----------------------
      end
      double precision function vnormd (n, v, w)
clll. optimize
c-----------------------------------------------------------------------
c this function routine computes the weighted root-mean-square norm
c of the vector of length n contained in the array v, with weights
c contained in the array w of length n..
c   vnormd = sqrt( (1/n) * sum( v(i)*w(i) )**2 )
c-----------------------------------------------------------------------
      integer n,   i
 ...
 10     sum = sum + (v(i)*w(i))**2
      vnormd = dsqrt(sum/dfloat(n))
      return
c----------------------- end of function vnormd -------------------------
      end
      subroutine xerrwd (msg, nmes, nerr, level, ni, i1, i2, nr, r1, r2)
      integer msg, nmes, nerr, level, ni, i1, i2, nr,
     1   i, lun, lunit, mesflg, mflag, ncpw, nch, nwds
      double precision r1, r2
      dimension msg(nmes)
c-----------------------------------------------------------------------
c subroutines xerrwd, xsetfd, and xsetund, as given here, constitute
...
c    in d21.13 format.
c 5. the common block /ehd001/ below is data-loaded (a machine-
c    dependent feature) with default values.
c    this block is needed for proper retention of parameters used by
c    this routine which the user can reset by calling xsetfd or xsetund.
...
c----------------------- end of subroutine xerrwd ----------------------
      ...
      subroutine srcomd (rsav, isav, job)
c-----------------------------------------------------------------------
c this routine saves or restores (depending on job) the contents of
c the common blocks lsd001 and ehd001, which are used internally
...
c
 ...
 120     ils(i) = isav(i)
      ...
      ieh(2) = isav(lenils+2)
      return
c----------------------- end of subroutine srcomd -----------------------
      end
cgy      subroutine xsetfd (mflag)
cgyc
cgyc this routine resets the print control flag mflag.
cgyc
cgy      integer mflag, mesflg, lunit
cgy      common /ehd001/ mesflg, lunit
cgyc
cgy      if (mflag .eq. 0 .or. mflag .eq. 1) mesflg = mflag
cgy      return
cgyc----------------------- end of subroutine xsetfd -----------------------
cgy      end
cgy      subroutine xsetund (lun)
cgyc
cgyc this routine resets the logical unit number for messages.
cgyc
cgy      integer lun, mesflg, lunit
cgy      common /ehd001/ mesflg, lunit
cgyc
cgy      if (lun .gt. 0) lunit = lun
cgy      return
cgyc----------------------- end of subroutine xsetund ----------------------
cgy      end
