...
c========================= Probing Tool End ============================
c
      integer inpdate,emsdate,ozndate,bnddate,wrtdate,enddate 
      integer iproc_id, nlayav
      real inptim,emstim,ozntim,bndtim,wrttim,endtim 
      character*20 version
      character*8 chtime, chdate
      logical lupdtdep(MXGRID)
c
      data version /'CAMx v6.00,05-06-13'/
c
c-----Entry point
c
...
c
     ...
     &              ozndate,bndtim,bnddate,wrttim,wrtdate,endtim,
     &              enddate,numprocs,iproc_id) 
      nlayav = nlay(1)
...
c
      if( io3col .NE. 0 .AND. idmech .NE. 10 ) 
...
c
     ...
     &                                         ptwetfld(ipsadep(igrd)))
             ...
             if( lsrfmod ) then
                if( lhdfout ) then
                   call hdf5_file_out('srfmod',date,time)
                else
                  do igrd = 1,ngrid
                    call wrtsrfrt(time,date,iowrtsrf(igrd),ncol(igrd),
     &                     nrow(igrd),ntotsp,rtsolmas(ipsa2d(igrd)),
