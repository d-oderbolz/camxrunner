...
c
c     Revision History:
c       05/22/08   Version 4.50 released (see Release_notes_4.51)
...
c
      data version /'CAMx v4.51, 08-05-22'/
c
c-----Entry point
c
      nsteps = 0
...
c
     ...
     &                  icdsno(iptr2d(igrd)),conc(iptr4d(igrd)),
     &                  vdep(iptrem(igrd)) )
...
c
         if( lwalls ) then
           do igrd=1,ngrid
             call avgwal(igrd,ncol(igrd),nrow(igrd),nlay(igrd),
