...
c        05/12/08   Revised deposited liquid concentration to use correct rainfall volume
c
c     Input arguments:
c        igrid               grid index 
...
c
              if( lipr .AND. ipa_cel(i,j,kbot) .GT. 0 ) then
                ipa_idx = ipa_cel(i,j,kbot)
                cipr(IPR_WDEP, ipa_idx, l) =
...
c
            ...
            area = deltax(j)*deltay/mapscl(i,j)**2
