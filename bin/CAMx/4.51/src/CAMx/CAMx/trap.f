     ...
     &                H2O,atm,O2,CH4,H2,conc,cncrad,avgcnc,avgrad,
...
c
c     Modifications:
c        01/22/02    Now has array to save average of radicals 
c                    over time step
c        09/19/05    N2O5 and HNO3 solved using Hertel approach
c
c     Input arguments:
...
cgy
      if (lddm) then
        write(*,*) 'Need to update DDM for new NO3/N2O5 solution'
        stop
      endif
cgy
c
...
c
      do l=1,ngas
        ctmp(l) = WT*conc(l) + (1.-WT)*cncold(l)
        avgcnc(l) = avgcnc(l) + (dt/dtin)*ctmp(l)
      enddo
      do l=1,nrad
