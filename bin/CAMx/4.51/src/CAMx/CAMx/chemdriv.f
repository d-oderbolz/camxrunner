...
c     It calls one of three ODE solvers: TRAP, IEHSOLV, or LSCALL.
...
c
      ...
      real      cold(MXSPEC), cnew(MXSPEC), o3old, o3new
c
...
c
      logical l3davg,ldark(ncol,nrow)
      real con(MXSPEC+1),crad(MXRADCL)
      real avgcnc(MXSPEC+1),avgrad(MXRADCL)
...
c$omp&  zenith,ldark,delcon,tcell,pcell,cliq,cldph,atm,O2,H2,CH4,
c$omp&  sddm,ldoipr,ldoirr,ipa_idx,titrt,irxn,rrxn_irr,convfac,
c$omp&  patmp,idx,nn,cold,cnew,o3old,o3new,avgcnc,avgrad,irt_cel,
c$omp&  iocean,hclc,cl2c,henso2,tfso2,heno3,tfo3,pm10,iht,ihg,
c$omp&  cldadj,ltrapirr,chg0,chg2,avgox,con_prec,con_CG)
c$omp&  copyin(/ijkgrd/)
c
...
c
            if ( aero_flag.gt.0 ) then
              avgox(1) = 0.5 * ( cncrad(i,j,k,kO  ) + crad(kO  ) )
              avgox(2) = 0.5 * ( cncrad(i,j,k,kOH ) + crad(kOH ) )
              avgox(3) = 0.5 * ( conc(i,j,k,kO3)/convfac + con(kO3) )
              avgox(4) = 0.5 * ( cncrad(i,j,k,kNO3) + crad(kNO3) )

              avgox(1) = amax1(avgox(1), bdlrad)
              avgox(2) = amax1(avgox(2), bdlrad)
              avgox(3) = amax1(avgox(3), bdnl(kO3))
              avgox(4) = amax1(avgox(4), bdlrad)
...
c
               if( ldark(i,j) .OR. delcon(2,ko3) .lt. desto3 ) then
                 desto3 = delcon(2,ko3)
                 prdo3n = 0.0
                 prdo3v = 0.0
               else
...
c
               if( tectyp .EQ. OSAT .OR.
     &                            (tectyp .EQ. PSAT .AND. lozone) ) then
                  call osatsa(igrd,i,j,k,
     &                               prdo3n,prdo3v,desto3,delcon,dtchem)
               else if( tectyp .EQ. GOAT ) then
                  call goatsa(igrd,i,j,k,
     &                               prdo3n,prdo3v,desto3,delcon,dtchem)
               else if( tectyp .EQ. APCA ) then
                  call apcasa(igrd,i,j,k,
     &                               prdo3n,prdo3v,desto3,delcon,dtchem)
               else if( tectyp .EQ. RTRAC .AND.
     ...
     &                 convfac,irt_cel)
               ...
               if( tectyp .EQ. PSAT ) then
                  ...
                  call psatsa(igrd,i,j,k,convfac,cold,delcon,rrxn_irr)
                  call repartsa(igrd,i,j,k,cnew,delcon)
               endif
            endif
c
c========================= Source Apportion End ========================
...
c
            if( lirr .AND. (l3davg .OR. k .EQ. 1) ) then
               ...
               dtfact = dt/dtout/60.
               if( idmech.EQ.3 ) then
                 call cpamech3(rrxn_irr,rk,dtfact,MXRXN,patmp,ntotsp,
     &                                                    nn,ldark(i,j))
               elseif( idmech.EQ.4 ) then
                 call cpamech4(rrxn_irr,rk,dtfact,MXRXN,patmp,ntotsp,
     &                                                    nn,ldark(i,j))
               elseif( idmech.EQ.5 ) then
                 call cpamech5(rrxn_irr,rk,dtfact,MXRXN,patmp,ntotsp,
     &                                                    nn,ldark(i,j))
               elseif( idmech.EQ.6 ) then
                 call cpamech6(rrxn_irr,rk,dtfact,MXRXN,patmp,ntotsp,
     &                                                    nn,ldark(i,j))
               endif
c
...
c
      if (aero_flag.EQ.2) then
        aero_dt(igrd) = 0.0
  92    call uptime(time_aero(igrd),date_aero(igrd),60.*dt_aero)
