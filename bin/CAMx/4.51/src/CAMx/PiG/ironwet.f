      subroutine ironwet(n,dx,dy,mapscl,cwc,pwr,pws,pwg,cph,temp,press,
     &                   puffvol,conpig,depth,dt,depfld,flxwet)
c
c----CAMx v4.51 080522
c
...
c
     ...
     &     dt,dx,dy,mapscl
      real conpig(MXSPEC+1,MXRECTR),depfld(2*MXSPEC)
      real*8 flxwet(MXSPEC)
c
      integer nr,l,isemptyf,isemptyc
      real rd,rhoh2o,rr,volrat,rainvol,rhoair,convfac,cmin,conc,
...
c
     ...
     &                 temp,cwat,depth,rhoair,conc,hlaw,diffrat(l),
...
c
          ...
          depfld(l) = depfld(l) + 1.e-2*delm/(dx*dy/mapscl**2)
          depfld(nspec+l) = depfld(nspec+l) +
     &                      1.e-9*(delm/rainvol)*dt/(60.*dtout)
 40     continue
c
...
c
     ...
     &                (1. + cph2o/roprt(kph2o)/vtf)**0.33333
              call scavrat(.true.,lcloud,lgraupl,tamin,rr,temp,0.,depth,
...
c
     ...
     &                      depth,rhoair,0.,0.,0.,0.,psize,roprt(l),
     &                      gscav,ascav)
