      subroutine avepig(igrd,dt,ncol,nrow,nlay,dx,dy,mapscl,height,
...
c
      ...
      include "filunit.com"
c
      integer ncol,nrow,nlay,nspav,nspc,igrd
      real tempk(ncol,nrow,nlay),press(ncol,nrow,nlay),
     &     avcnc(ncol,nrow,nlay,nspav),height(ncol,nrow,nlay),
     &     dx(nrow),dy,mapscl(ncol,nrow)
      real vdmpvec(MXLAYA,MXSPEC),pcon
      integer lmap(nspc)
...
c
        call virtdump(n,ncol,nrow,nlay,nspc,i,j,dx(j),dy,
