...
c
      ...
      real avgrad(MXRADCL),avgcnc(MXSPEC+1),rrxn(MXRXN)
...
c
      if (idmech.eq.1) then
        call lscall(lsrate1,dtchem,wpuff,atm,O2,CH4,H2,con,crad,
...
c
     ...
     &      con(kcg6) .lt. bdnl(kcg6) .or.
     &      con(kcg7) .lt. bdnl(kcg7)) ierr = 1
        do is = ngas+1,nspec
          if (con(is).lt.bdnl(is)) ierr = 1
        enddo
