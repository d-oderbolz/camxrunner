...
c
      ...
      real         ttime, lefcls(MXTRCLS,MXCOLA,MXROWA)
      real         emscls(MXTRCLS,MXCOLA,MXROWA)
c
c-----------------------------------------------------------------------
c    Common blocks:
c-----------------------------------------------------------------------
...
c
      do igroup = 0,ngroup
         call rdargrp(igrid,ndate,ttime,nox,noy,igroup,emscls)
c
...
c
      if( ngroup .GT. 0 .AND. leftovr ) then
          do 50 j=2,noy-1
            ...
            iendcl = nox-1
            if( igrid .EQ. 1 ) then
               if( ibeg(j) .EQ. -999 ) goto 50
               ibegcl = ibeg(j)
               iendcl = iend(j)
            endif
            do 60 i=ibegcl,iendcl
c
c  --- get the region for this cell from mapping array ----
c
                imap = igrmap(igrid,i,j)
