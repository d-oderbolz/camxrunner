...
c
     ...
     &                 v1d(l) = 2.*v1d(l)/(mapscl(i+1,j) + mapscl(i,j))
            ...
            nn = i2 - i1 + 1
            if (iadvct.eq.2) then
              call hadvbot(nn,deltat,dx(j),c1d,v1d,m1d,flxarr,fpc,fmc)
            elseif (iadvct.eq.3) then
              call hadvppm(nn,deltat,dx(j),c1d,v1d,m1d,flxarr)
            endif
c
            l = 1
            do i = i1+1,i2-1
...
c
            if (iadvct.eq.2) then
              call hadvbot(nn,deltat,dy,c1d,v1d,m1d,flxarr,fpc,fmc)
            elseif (iadvct.eq.3) then
              call hadvppm(nn,deltat,dy,c1d,v1d,m1d,flxarr)
            endif
c
            l = 1
            do j = j1+1,j2-1
