      subroutine soap(ntot,caer,cgas,apoly,sopa,sopb,tempk,convfac,
     &                dt,iout,igrdchm,ichm,jchm,kchm,lppm,
     &                cpre,mwpre,csatT)
      implicit none
c
c**********************************************************************
...
c  MODIFICATIONS
c
c     Approximate solutions if POA is dominant - bkoo (4/13/2010)
c     Revised for the updated SOA scheme by bkoo (12/29/2006)
...
c     sopa    - polymerized SOA (anthropogenic) (ug/m3)
c     sopb    - polymerized SOA (biogenic) (ug/m3)
...
c
      include 'soap.inc'
      real        conmin, cpremin, xtola, xtolr
      parameter ( conmin  = 1.e-6 )
      parameter ( cpremin = 1.01e-9 )
      parameter ( xtola   = 1.e-8 )
      parameter ( xtolr   = 1.e-6 )
c
      integer     ntot
      ...
      integer     idx(NSOAP)
c
      real        apoly(ntot), sopa, sopb, dt, fpoly
...
c
      ...
      cpx = cpre/mwpre
c
c Add SOPA & SOPB to the pre-existing organic mass
c
      cpre = cpre + sopa + sopb
      cpx = cpx + sopa/mwsopa + sopb/mwsopb
c
...
c
 ...
 50   write (iout,'(a,i10,a,3i4)') ' Grid = ', igrdchm,
     &                 ' cell(i,j,k) = ', ichm, jchm, kchm
      ...
      write (iout,'(i5,2e)') (idx(i),sctot(i),scsat(i),i=1,nsol)
      write (iout,'(a,2e)') ' cpre,cpx ',cpre,cpx
      call camxerr()
c
c Converged
c
...
c
      fpoly = amin1( fpoly0 * dt, 1.0 ) ! fraction of polymerization
      do i=1,NASOA                  ! anthropogenic
        apoly(i) = fpoly * caer(i)
        caer(i) = caer(i) - apoly(i)
        sopa = sopa + apoly(i)
      enddo
      do i=NASOA+1,ntot-1           ! biogenic [except SQT]
        apoly(i) = fpoly * caer(i)
        caer(i) = caer(i) - apoly(i)
        sopb = sopb + apoly(i)
      enddo
      apoly(ntot) = 0.0             ! no polymerization for SQT
c
