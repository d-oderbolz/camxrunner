     ...
     &                   cloud,cldwtr,ranwtr,snowtr,gplwtr,cldod,energy,
     &                   cldph)
      use filunit
      use grid
      use camxfld
...
c     - Water vapor fields     at next update date/time, all grids (optional)
c     - Kv fields              at next update date/time, all grids (optional)
...
c
     ...
     &     energy(ngcol,ngrow,nglay),cldph(ngcol,ngrow,nglay)
...
c
c-----Read height/pressure file for master grid and optionally for any fine
c     grids
c
      iunit = ihtp(igrd)
      if( iunit .GT. 0 ) then
...
c             
     ...
     &           'Past expected time/date ',htim,hdate
           if( igrd .EQ. 1 ) then
              string = 'Reading height/pressure file for master grid.'
           else
              write(string,'(A,I5)')
...
c
c-----Read wind file for master grid and optionally for any fine grids
c
      iunit = iwind(igrd)
      if (iunit.gt.0) then
 201    continue
...
c             
     ...
     &          'Past current time/date ',time,date
          if( igrd .EQ. 1 ) then
              string = 'Reading wind file for master grid.'
          else
...
c
c-----Calculate time rates of change for u and v winds
c
        call timrates(ngcol,ngrow,nglay,wndu,unext,puptim)
        call timrates(ngcol,ngrow,nglay,wndv,vnext,pvptim)
      endif
c
c-----Read temperature file for master grid and optionally any fine grids
c
      iunit = itemp(igrd)
      if (iunit.gt.0) then
...
c             
     ...
     &          'Past expected time/date ',htim,hdate
          if( igrd .EQ. 1 ) then
              string = 'Reading temperature file for master grid.'
          else
              write(string,'(A,I5)')
...
c
c-----Read water vapor file for master grid and optionally any fine grids
c
 ...
 401      do k = 1,nglay
            read(iunit,end=7000) hr,idt,
     &               ((wnext(i,j,k),i=1,ngcol),j=1,ngrow)
          enddo
c
          if (.not.ly2k .and. idt.gt.100000) call juldate(idt)
          if (hr.ge.2400.) then
     ...
     &          'Read water vapor file at ',hr,idt,' grid',igrd
          call flush(iout)
c
          if (idt.lt.hdate .or. (idt.eq.hdate .and. hr.lt.htim))
     &                                                  goto 401 
          if (idt.gt.hdate .or. (idt.eq.hdate .and. hr.gt.htim)) then
            write(iout,'(//,a)') 'ERROR in READINP:'
            write(iout,'(a,f10.1,i10.5)')
     &            'Past expected time/date ',htim,hdate
            if( igrd .EQ. 1 ) then
              string = 'Reading water vapor file for master grid.'
            else
              write(string,'(A,I5)')
...
c
c-----Calculate time rates of change for 3-D water vapor
c
          call timrates(ngcol,ngrow,nglay,vapor,wnext,pwptim)
        endif
      endif
c
c-----Read cloud file for master grid and optionally any fine grids
c
      if (lchem .or. lwet .or. ldry) then
        iunit = icld(igrd)
...
c
 501      read(iunit,end=7000,err=7001) hr,idt
     ...
     &           ((cldwtr(i,j,k),i=1,ngcol),j=1,ngrow)
            read(iunit,end=7000,err=7001)
     &           ((ranwtr(i,j,k),i=1,ngcol),j=1,ngrow)
            read(iunit,end=7000,err=7001)
     &           ((snowtr(i,j,k),i=1,ngcol),j=1,ngrow)
            read(iunit,end=7000,err=7001)
     &           ((gplwtr(i,j,k),i=1,ngcol),j=1,ngrow)
            read(iunit,end=7000,err=7001)
...
c
     ...
     &          'Read cloud/rain file at ',hr,idt,' grid',igrd
          call flush(iout)
c               
          if (idt.lt.date .or.(idt.eq.date .and. hr.lt.atim)) goto 501
          if (idt.gt.date .or. (idt.eq.date .and. hr.gt.atim)) then
            write(iout,'(//,a)') 'ERROR in READINP:'
            write(iout,'(a,f10.1,i10.5)')
     &            'Past current time/date ',time,date 
            if( igrd .EQ. 1 ) then
              string = 'Reading cloud/rain file for master grid.'
            else
              write(string,'(A,I5)')
...
c
c-----Calculate energy transmission coefficient using RADM cloud adjustment
c
          do k = 1,nglay
            do j = 1,ngrow
              do i = 1,ngcol
          ...
          if (.not.ltuv) then
            do j = 1,ngrow
              do i = 1,ngcol
                if (lcld53) then
                  tau(nglay) = cldod(i,j,nglay)
                  do k = nglay-1,1,-1
                    tau(k) = tau(k+1) + cldod(i,j,k)
                  enddo
                  do k = 1,nglay
                    cldod(i,j,k) = tau(k)
                  enddo
                ...
                do k = 1,nglay
                  if (cldod(i,j,k).lt.5.) then
                    energy(i,j,k) = 1.
                    cloud(i,j,k) = 0.
                  else
                    energy(i,j,k) = (5. - exp(-cldod(i,j,k)))/
     &                              (4. + 0.42*cldod(i,j,k))
                    cloud(i,j,k) = 1.
                  endif
                enddo
...
c
c-----Read vertical diffusion coefficient for master grid and optionally
...
c
 ...
 701    do k = 1,nglay
          read(iunit,end=7000) hr,idt,
     &               ((knext(i,j,k),i=1,ngcol),j=1,ngrow)
        enddo
c
        if (.not.ly2k .and. idt.gt.100000) call juldate(idt)
        if (hr.ge.2400.) then
     ...
     &        'Read KV file at ',hr,idt,' grid',igrd
        call flush(iout)
c             
        if (idt.lt.hdate .or. (idt.eq.hdate .and. hr.lt.htim)) goto 701 
        if (idt.gt.hdate .or. (idt.eq.hdate .and. hr.gt.htim)) then 
          write(iout,'(//,a)') 'ERROR in READINP:'
          write(iout,'(a,f10.1,i10.5)')
     &          'Past expected time/date ',htim,hdate
           if( igrd .EQ. 1 ) then
              string = 'Reading diffusivity file for master grid.'
           else
              write(string,'(A,I5)')
...
c
c-----Calculate time rates of change for 3-D vertical diffusivity
c
...
c
 7001 continue
      ...
      write(iout,*)'There was an error reading a record of the'
      write(iout,*)'cloud/rain file.  This file has a new format'
      write(iout,*)'so make sure it contains records for:'
      write(iout,*)'cloud water content'
      write(iout,*)'rain water content'
      write(iout,*)'snow water content'
      write(iout,*)'graupel water content'
      write(iout,*)'cloud optical depth'
      call camxerr()
c
 9999 continue
      return
