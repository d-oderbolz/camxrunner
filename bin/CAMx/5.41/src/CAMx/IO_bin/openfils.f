...
c
         ...
         call jstlft( cldhdr )
         call toupper( cldhdr )
         if( cldhdr .EQ. 'CAMX_V4.3 CLOUD_RAIN' .or.
     &       cldhdr .EQ. 'CAMX_V5.3 CLOUD_RAIN' ) then
            backspace(icld(1))
            n = 1
     ...
     &                               nzcl.ne.nlay(1)) goto 7007
            lcld53 = .false.
            if (cldhdr .EQ. 'CAMX_V5.3 CLOUD_RAIN' ) lcld53 = .true.
            if (.not.lcld53 .and. ltuv) goto 7009
         else
           goto 7006 
         endif
c
...
c
 7006 continue
      ...
      write(iout,'(2A)') 'Expecting: CAMx_V4.3 CLOUD_RAIN or ',
     &                                  'CAMX_V5.3 CLOUD_RAIN'
...
c
 7009 continue
      write(iout,'(//,a)') 'ERROR in OPENFILS:'
      write(iout,'(2A)') 'CAMx cloud/rain file header is: ',
     &                                      'CAMx_V4.3 CLOUD_RAIN'
      write(iout,'(A)')'You cannot use the in-line TUV adjustment.'
      write(iout,'(2A)')'Either provide a CAMx_V5.3 CLOUD_RAIN file ',
     &                                           'or set the variables '
      write(iout,'(2A)') 'TUV_Cloud_Adjust and TUV_Aero_Adjust in',
     &                   ' your CAMx control file to false.'
      call camxerr()
c
 7010 continue
      write(iout,'(//,a)') 'ERROR in OPENFILS:'
      write(iout,'(2A)') 'The chemistry flag is set but no ',
     &                         'Albedo_Haze_Ozone file is supplied.'
      write(iout,'(2A)')'Either supply an Albedo_Haze_Ozone file ',
     &                         'or set the Chemistry flag to false.'
      write(iout,'(2A)')'If using Mechanism 10 just supply ',
     &                  'a dummy file for the Albedo_Haze_Ozone file.'
      call camxerr()
c
 9999 continue
      return
