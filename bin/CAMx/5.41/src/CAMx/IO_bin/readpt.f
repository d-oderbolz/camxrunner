...
c
      do n=1,npts
        if(hstk(n) .LT. 0. .AND. effph(n) .GE. 0. ) then
           write(iout,'(//,a)')'ERROR in READPT:'
           write(iout,*) 'Invalid values found for stack height and plume height.'
           write(iout,*) 'The stack height is negative, triggering the plume ',
...
c
        if(hstk(n) .LT. 0. .AND. effph(n) .LT. 0. ) then
           if( ABS(effph(n)) .LE. ABS(hstk(n)) ) then
              write(iout,'(//,a)')'ERROR in READPT:'
              write(iout,*) 'Invalid values found for stack height and plume height.'
              write(iout,*) 'The plume distribution override is triggered. But the ',
     &               'value for stack height is larger than value for plume height.'
              call camxerr()
           endif
        endif
      enddo
