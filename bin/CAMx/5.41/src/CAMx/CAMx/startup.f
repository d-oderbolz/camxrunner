...
c
      if( iaho .NE. 0 .AND. idmech .NE. 10 ) call ahoprep()
c
...
c
c   --- call routine to write the header of the coarse grid
c       average surface tracer deposition file ---
c
...
c
         if( idmech .LT. 2 .OR. idmech .GT. 9 ) then
            write(iout,'(//,a)') 'ERROR in STARTUP:'
...
c
 7001 continue
      write(iout,'(//,A)') 'ERROR in STARTUP:'
      write(iout,*) 'A parameter in the tracer.inc is not ',
     &                                        'sufficiently large.'
      write(iout,*) 'Please change the value for parameter: ',cparm
      write(iout,*) 'It should be set to a value of at least: ',icheck
      call flush(iout)
