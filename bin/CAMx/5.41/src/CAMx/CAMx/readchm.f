...
c
      ...
      character*10     fasts8(34), fasts9(24)
      ...
      real             bdnl_tmp,roprt_tmp,bext_tmp,ssa_tmp
...
c     The IEH fast species must be defined in FASTS lists
c     Radicals must be listed before non-radical species in FASTS lists
c
...
c
      ...
      data mchaero  /  0, 22,  0,  0, 22, 22, 22,  0,  0,999 /
      data mchrad   / 26, 20,  0,  0, 16, 13, 23, 19, 16,  0 /
      data mchiessr /  2,  2,  0,  0,  2,  2,  2,  4,  4,  0 /
      data mchrxn   /251,213,  0,  0,217,156,218,288,222,  0 /
      data mchphot  / 39, 29,  0,  0, 30, 23, 28, 45, 38,  0 /
      data mchfast  / 35, 29,  0,  0, 31, 21, 32, 34, 24,  0 /
...
c
     ...
     &      ' Number of aerosols = 0 for CF, CMU or INERT PM option'
      ...
      if (naero.EQ.0 .AND. ltuva) then
        write(iout,'(//A,A)')
     &      ' Number of aerosols = 0 but you chose in-line TUV aerosol',
     &      ' adjustment'
        write(iout,'(a)') ' Set TUV_Aero_Adjust to false'
        goto 910
      endif
      if (naero.GT.0) then
