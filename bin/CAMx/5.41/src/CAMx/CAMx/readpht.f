...
c
      character*80 record
      real         tmp3(3)
      integer      line, iozn, ialb, ihaze, kount
...
c
  ...
  11        continue
            ...
            albcl(ialb) = tmp3(2)
            hazcl(ihaze) = tmp3(3)
            if (ltuva .AND. hazcl(1).ne.0.) goto 7004
c
            do ihght = 1,NHGHT
              line = line + 1
...
c
 7004 continue
     ...
     &  ' You chose to run in-line TUV adjustment for aersols',
     &  ' but clear-sky photolysis file has NON-ZERO haze OD:',
     &  ' To avoid double-counting haze impacts on photolysis,',
     &  ' either set TUV_Aero_adjust to false in the CAMx control file',
     &  ' OR re-run AHOMAP and TUV pre-processors with 1st Haze OD = 0'
      call camxerr()
c
c----------------------------------------------------------------------
c    Return point:
