...
c
      if (ipigflg .NE. 0 .AND. ipigint .EQ. igrd) then
        if( iproc_id .eq. 0 ) then
           ...
           call piginit(deltat(igrd),nptsrc,nrad,nspec,ptemis,height,
...
c
     ...
     &                      lrdruf(igrd),icdruf(iptr2d(igrd)),ruflen(1),
     &                      lrdsno,icdsno(iptr2d(igrd)),
...
c
     ...
     &          depdum,fsurf(iptrlu(igrd)),ipsa3d(igrd),tarray2,
...
c
     ...
     &                  fsurf(iptrlu(igrd)),ipsa3d(igrd),tarray2,
     &                  iproc_id,ipacl_2d(iptr2d_full(igrd)),
     &                  ipacl_3d(iptr3d_full(igrd)),
