...
c
     ...
     & Wet_Deposition,ACM2_Diffusion,Staggered_Winds,
     & Gridded_Emissions,Point_Emissions,Ignore_Emission_Dates,
     & TUV_Cloud_Adjust,TUV_Aero_Adjust,Root_Output_Name,
...
c 
      ...
      ltuvc  = TUV_Cloud_Adjust
      ltuva  = TUV_Aero_Adjust
      ...
      ltuv   = .false.
      if (ltuvc .OR. ltuva) ltuv = .true.
      if (ltuva .AND. .not.ltuvc) then
        write(iout,'(//,a)') 'ERROR in READNML:'
        write(iout,'(a,/,a,/,a,/,2a)')
     &         'You have elected in-line TUV photolysis adjustment',
     &         'for aerosols, but not for clouds.  CAMx allows TUV',
     &         'to be run for clouds alone or both clouds and aerosol.',
     &         'Either turn on TUV_Cloud_Adjust or turn off',
     &         ' TUV_Aero_Adjust.'
        call camxerr()
      endif
...
c
      ...
      if( navspc .GT. 0 ) then
c
c  --- call routine to alloacte the arrays ---
c
...
c
     ...
     &         'ACM2 Vertical Diffusion    : ',lacm2
      write(idiag,'(A,L10)')
     &         'TUV Cloud Adjustment       : ',ltuvc
      write(idiag,'(A,L10)')
     &         'TUV Aero Adjustment        : ',ltuva
      write(idiag,'(A,L10)') 
