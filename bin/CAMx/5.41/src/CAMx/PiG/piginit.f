      subroutine piginit(dt,npts,nrads,nspc,pttrace,height,windu,
     &                                         windv,tempk,press)
      use filunit
      use grid
      use chmstry
...
c
        if( hstk(n) .LE. 0.  ) then
            ...
            write(iout,*) 'Found negative stack height for a PiGged source.'
            write(iout,*) 'The plume distribution override is not ',
     &                  'allowed for PiGged sources.'
            write(iout,*) 'Either reset the stack height ',
