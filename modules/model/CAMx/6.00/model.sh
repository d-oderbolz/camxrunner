...
#
# Version: $Id: model.sh 8649 2012-07-10 15:01:13Z oderbolz $ 
#
...
CXR_META_MODULE_VERSION='$Id: model.sh 8649 2012-07-10 15:01:13Z oderbolz $'
...
#
# Sets the variables (on a daily basis), taking into account if we use the scratch-
# dir or not. 
#
# Parameters:
# $1 - create_missing_dirs, if false (deafult: true), common.runner.evaluateRule will not create missing dirs
################################################################################	
function set_variables() 
################################################################################
{
	...
	CXR_RT_ROOT_OUTPUT=$(common.runner.evaluateRule "$CXR_RT_ROOT_OUTPUT_FILE_RULE" false CXR_RT_ROOT_OUTPUT_FILE_RULE true $create_missing_dirs)
	CXR_SA_ROOT_OUTPUT=$(common.runner.evaluateRule "$CXR_SA_ROOT_OUTPUT_FILE_RULE" false CXR_SA_ROOT_OUTPUT_FILE_RULE true $create_missing_dirs)
	...
	CXR_MODEL_CTRL_FILE=$(common.runner.evaluateRule "$CXR_MODEL_CTRL_FILE_RULE" false CXR_MODEL_CTRL_FILE_RULE true $create_missing_dirs)
	
	if [[ "$CXR_PROBING_TOOL" == "OSAT" || "$CXR_PROBING_TOOL" == "PSAT" ]]
	then
		CXR_SA_RECEPTORIN=$(common.runner.evaluateRule "$CXR_SA_RECEPTOR_DEFINITIONS_FILE_RULE" false CXR_SA_RECEPTOR_DEFINITIONS_FILE_RULE true $create_missing_dirs)
	fi
	...
	then	
		CXR_TOP_CONCENTRATIONS_INPUT_FILE=$(common.runner.evaluateRule "$CXR_TOP_CONCENTRATIONS_FILE_RULE" false CXR_TOP_CONCENTRATIONS_FILE_RULE true $create_missing_dirs)
	else
		# Starting with version 5.20.1, CAMx needs no TOPCONC anymore
		CXR_TOP_CONCENTRATIONS_INPUT_FILE=
	fi

	if [[ "$(common.math.compareVersions "$CXR_MODEL_VERSION" "6.00" )" -eq 1 ]]
	then
		CXR_ALBEDO_HAZE_OZONE_INPUT_FILE=$(common.runner.evaluateRule "$CXR_ALBEDO_HAZE_OZONE_FILE_RULE" false CXR_ALBEDO_HAZE_OZONE_FILE_RULE true $create_missing_dirs)
    else
		# Starting with version 6.00, CAMx needs just an ozone map file. 
		CXR_O3MAP_INPUT_FILE=$(common.runner.evaluateRule "$CXR_O3MAP_FILE_RULE" false CXR_O3MAP_FILE_RULE true $create_missing_dirs)
    fi
	
	# Checks we need always
	if [[ "$(common.math.compareVersions "$CXR_MODEL_VERSION" "6.00" )" -eq 1 ]]
	then
		CXR_CHECK_THESE_INPUT_FILES="$CXR_CHECK_THESE_INPUT_FILES $CXR_PHOTOLYIS_RATES_INPUT_FILE $CXR_BOUNDARY_CONDITIONS_INPUT_FILE $CXR_TOP_CONCENTRATIONS_INPUT_FILE $CXR_ALBEDO_HAZE_OZONE_INPUT_FILE $CXR_O3MAP_INPUT_FILE"
	else
		CXR_CHECK_THESE_INPUT_FILES="$CXR_CHECK_THESE_INPUT_FILES $CXR_PHOTOLYIS_RATES_INPUT_FILE $CXR_BOUNDARY_CONDITIONS_INPUT_FILE $CXR_TOP_CONCENTRATIONS_INPUT_FILE $CXR_O3MAP_INPUT_FILE"
	fi
	# PiG
	if [[ "$CXR_PLUME_IN_GRID" == true  ]]
	then
		CXR_POINT_SOURCES_INPUT_FILE=$(common.runner.evaluateRule "$CXR_POINT_SOURCES_FILE_RULE" false CXR_POINT_SOURCES_FILE_RULE true $create_missing_dirs)
	...
	then
		...
		CXR_MASTER_GRID_RESTART_INPUT_FILE=$(common.runner.evaluateRule "$CXR_MASTER_GRID_RESTART_FILE_RULE" false CXR_MASTER_GRID_RESTART_FILE_RULE true $create_missing_dirs)
		
		if [[ $CXR_NUMBER_OF_GRIDS -gt 1 ]]
		then
			CXR_NESTED_GRID_RESTART_INPUT_FILE=$(common.runner.evaluateRule "$CXR_NESTED_GRID_RESTART_FILE_RULE" false CXR_NESTED_GRID_RESTART_FILE_RULE true $create_missing_dirs)
		else
		...
		if [[ "$CXR_PLUME_IN_GRID" == true ]]
		then
			CXR_PIG_RESTART_INPUT_FILE=$(common.runner.evaluateRule "$CXR_PIG_RESTART_FILE_RULE" false CXR_PIG_RESTART_FILE_RULE true $create_missing_dirs)
		fi # PiG?
	
		if [[ "$CXR_PROBING_TOOL" == "OSAT" || "$CXR_PROBING_TOOL" == "PSAT" || "$CXR_PROBING_TOOL" == "GOAT" || "$CXR_PROBING_TOOL" == "APCA" ]] 
		then
			CXR_SA_MASTER_RESTART_INPUT_FILE=$(common.runner.evaluateRule "$CXR_SA_MASTER_RESTART_FILE_RULE" false CXR_SA_MASTER_RESTART_FILE_RULE true $create_missing_dirs)
			
			if [[ $CXR_NUMBER_OF_GRIDS -gt 1 ]]
			then
				CXR_SA_NESTED_RESTART_INPUT_FILE=$(common.runner.evaluateRule "$CXR_SA_NESTED_RESTART_FILE_RULE" false CXR_SA_NESTED_RESTART_FILE_RULE true $create_missing_dirs)
			else
		...
		# Set the optional ones "" per default
	...
	then
		...
		CXR_CHECK_THESE_OUTPUT_FILES="$CXR_CHECK_THESE_OUTPUT_FILES $(common.runner.evaluateRule "$CXR_SA_INST_FILE_RULE" false CXR_SA_INST_FILE_RULE false $create_missing_dirs)"	
		# Add nested, if needed
		if [[ $CXR_NUMBER_OF_GRIDS -gt 1 ]]
		then
			CXR_CHECK_THESE_OUTPUT_FILES="$CXR_CHECK_THESE_OUTPUT_FILES $(common.runner.evaluateRule "$CXR_SA_FINST_FILE_RULE" false CXR_SA_FINST_FILE_RULE false $create_missing_dirs)"	
		fi
		...
		do
			##Source Area
			CXR_SA_SOURCE_AREA_MAP_INPUT_ARR_FILES[${CXR_ISRCREGION}]=$(common.runner.evaluateRule "$CXR_SA_SOURCE_AREA_MAP_FILE_RULE" false CXR_SA_SOURCE_AREA_MAP_FILE_RULE true $create_missing_dirs) 
		...
		do
			...
			# We only want to check them, otherwise we dont need these values
			CXR_CHECK_THESE_OUTPUT_FILES="$CXR_CHECK_THESE_OUTPUT_FILES $(common.runner.evaluateRule "$CXR_SA_AVG_FILE_RULE" false CXR_SA_AVG_FILE_RULE false $create_missing_dirs)"
			
			##Source Group
			for CXR_ISRCGROUP in $(seq 1 $CXR_SA_NUMBER_OF_SOURCE_GROUPS);
			do
				eval CXR_SA_EMISS_GROUP_GRID_INPUT_${CXR_IGRID}_${CXR_ISRCGROUP}=$(common.runner.evaluateRule "$CXR_SA_EMISS_GROUP_GRID_INPUT_FILE_RULE" false CXR_SA_EMISS_GROUP_GRID_INPUT_FILE_RULE true $create_missing_dirs)
		...
		then
			for CXR_ISRCGROUP in $(seq 1 $CXR_SA_NUMBER_OF_SOURCE_GROUPS);
			do
				CXR_SA_POINTS_GROUP_INPUT_ARR_FILES[${CXR_ISRCGROUP}]=$(common.runner.evaluateRule "$CXR_SA_POINTS_GROUP_FILE_RULE" false CXR_SA_POINTS_GROUP_FILE_RULE true $create_missing_dirs)
	...
	then
		...
		CXR_DDM_ROOT_OUTPUT=$(common.runner.evaluateRule "$CXR_DDM_ROOT_OUTPUT_FILE_RULE" false CXR_DDM_ROOT_OUTPUT_FILE_RULE true $create_missing_dirs)
		
		CXR_DDM_MASTER_RESTART_INPUT_FILE=$(common.runner.evaluateRule "$CXR_DDM_MASTER_RESTART_FILE_RULE" false CXR_DDM_MASTER_RESTART_FILE_RULE true $create_missing_dirs)
		
		if [[ $CXR_NUMBER_OF_GRIDS -gt 1 ]]
		then
			CXR_DDM_NESTED_RESTART_INPUT_FILE=$(common.runner.evaluateRule "$CXR_DDM_NESTED_RESTART_FILE_RULE" false CXR_DDM_NESTED_RESTART_FILE_RULE true $create_missing_dirs)
		else
		...
		for CXR_IGRID in $(seq 1 $CXR_NUMBER_OF_GRIDS);
		do
			CXR_DDM_SOURCE_AREA_MAP_INPUT_ARR_FILES[${CXR_IGRID}]=$(common.runner.evaluateRule "$CXR_DDM_SOURCE_AREA_MAP_FILE_RULE" false CXR_DDM_SOURCE_AREA_MAP_FILE_RULE true $create_missing_dirs) 
		...
		do
			CXR_SA_POINTS_GROUP_INPUT_ARR_FILES[${CXR_IGRID}]=$(common.runner.evaluateRule "$CXR_SA_POINTS_GROUP_INPUT_FILE_RULE" false CXR_SA_POINTS_GROUP_INPUT_FILE_RULE true $create_missing_dirs) 
		...
		do
			...
			do
				...
				CXR_DDM_EMISS_GROUP_GRID_${j}_${CXR_IGRID}_INPUT_FILE=$(common.runner.evaluateRule "${!ELEMENT_NAME}" false $ELEMENT_NAME true $create_missing_dirs) 
	...
	then
		CXR_RT_MASTER_RESTART_INPUT_FILE=$(common.runner.evaluateRule "$CXR_RT_MASTER_RESTART_FILE_RULE" false CXR_RT_MASTER_RESTART_FILE_RULE true $create_missing_dirs)
		
		if [[ $CXR_NUMBER_OF_GRIDS -gt 1 ]]
		then
			CXR_RT_NESTED_RESTART_INPUT_FILE=$(common.runner.evaluateRule "$CXR_RT_NESTED_RESTART_FILE_RULE" false CXR_RT_NESTED_RESTART_FILE_RULE true $create_missing_dirs)
		else
	...
	then
		# This is not a file (hence no _FILE at the end of the name)
		CXR_PA_ROOT_OUTPUT=$(common.runner.evaluateRule "$CXR_PA_ROOT_OUTPUT_FILE_RULE" false CXR_PA_ROOT_OUTPUT_FILE_RULE true $create_missing_dirs)
	fi # General Probing
	...
	# Output files must not be decompressed!
	CXR_DIAG_OUTPUT_FILE=$(common.runner.evaluateRule "$CXR_DIAG_FILE_RULE" false CXR_DIAG_FILE_RULE false $create_missing_dirs)
	
	if [[ $CXR_NUMBER_OF_GRIDS -gt 1 ]]
	then
		CXR_FINST_OUTPUT_FILE=$(common.runner.evaluateRule "$CXR_FINST_FILE_RULE" false CXR_FINST_FILE_RULE false $create_missing_dirs)
	else
		CXR_FINST_OUTPUT_FILE=""
	fi
	
	CXR_INST_OUTPUT_FILE=$(common.runner.evaluateRule "$CXR_INST_FILE_RULE" false CXR_INST_FILE_RULE false $create_missing_dirs)
	CXR_MASS_OUTPUT_FILE=$(common.runner.evaluateRule "$CXR_MASS_FILE_RULE" false CXR_MASS_FILE_RULE false $create_missing_dirs)
	CXR_OUT_OUTPUT_FILE=$(common.runner.evaluateRule "$CXR_OUT_FILE_RULE" false CXR_OUT_FILE_RULE false $create_missing_dirs)
	...
	for CXR_IGRID in $(seq 1 ${CXR_NUMBER_OF_GRIDS});
	do
		# Landuse pre-CAMx-v6.00
		CXR_LANDUSE_GRID_INPUT_ARR_FILES[${CXR_IGRID}]=$(common.runner.evaluateRule "$CXR_LANDUSE_FILE_RULE" false CXR_LANDUSE_FILE_RULE true $create_missing_dirs)
		# Pressure pre-CAMx-v6.00
		CXR_ZP_GRID_INPUT_ARR_FILES[${CXR_IGRID}]=$(common.runner.evaluateRule "$CXR_PRESSURE_FILE_RULE" false CXR_PRESSURE_FILE_RULE true $create_missing_dirs)
		# Wind pre-CAMx-v6.00
		CXR_WIND_GRID_INPUT_ARR_FILES[${CXR_IGRID}]=$(common.runner.evaluateRule "$CXR_WIND_FILE_RULE" false CXR_WIND_FILE_RULE true $create_missing_dirs)
		# Temperature pre-CAMx-v6.00
		CXR_TEMP_GRID_INPUT_ARR_FILES[${CXR_IGRID}]=$(common.runner.evaluateRule "$CXR_TEMPERATURE_FILE_RULE" false CXR_TEMPERATURE_FILE_RULE true $create_missing_dirs)
		# Vapor pre-CAMx-v6.00
		CXR_VAPOR_INPUT_ARR_FILES[${CXR_IGRID}]=$(common.runner.evaluateRule "$CXR_VAPOR_FILE_RULE" false CXR_VAPOR_FILE_RULE true $create_missing_dirs)
		# Cloud pre-CAMx-v6.00
		CXR_CLOUD_GRID_INPUT_ARR_FILES[${CXR_IGRID}]=$(common.runner.evaluateRule "$CXR_CLOUD_FILE_RULE" false CXR_CLOUD_FILE_RULE true $create_missing_dirs)
		# Vertical K pre-CAMx-v6.00
		CXR_KV_GRID_INPUT_ARR_FILES[${CXR_IGRID}]=$(common.runner.evaluateRule "$CXR_K_FILE_RULE" false CXR_K_FILE_RULE true $create_missing_dirs)
		# Emissions
		CXR_EMISS_INPUT_ARR_FILES[${CXR_IGRID}]=$(common.runner.evaluateRule "$CXR_EMISSION_FILE_RULE" false CXR_EMISSION_FILE_RULE true $create_missing_dirs)
		# Landuse CAMx-v6.00
		CXR_2D_SFC_CAMXV6_INPUT_ARR_FILE[${CXR_IGRID}]=$(common.runner.evaluateRule "$CXR_2D_SFC_FILE_RULE_CAMXV6" false CXR_2D_SFC_FILE_RULE_CAMXV6 true $create_missing_dirs)
		# 3d Meteo CAMx-v6.00
		CXR_3D_MET_CAMXV6_INPUT_ARR_FILE[${CXR_IGRID}]=$(common.runner.evaluateRule "$CXR_3D_MET_FILE_RULE_CAMXV6" false CXR_3D_MET_FILE_RULE_CAMXV6 true $create_missing_dirs)
		# 2d Meteo CAMx-v6.00
		CXR_2D_MET_CAMXV6_INPUT_ARR_FILE[${CXR_IGRID}]=$(common.runner.evaluateRule "$CXR_2D_MET_FILE_RULE_CAMXV6" false CXR_2D_MET_FILE_RULE_CAMXV6 true $create_missing_dirs)
		# 3d Kv CAMx-v6.00
		CXR_3D_KV_CAMXV6_INPUT_ARR_FILE[${CXR_IGRID}]=$(common.runner.evaluateRule "$CXR_3D_KV_FILE_RULE_CAMXV6" false CXR_3D_KV_FILE_RULE_CAMXV6 true $create_missing_dirs)
		# 3d Cloud CAMx-v6.00
		CXR_3D_CLD_CAMXV6_INPUT_ARR_FILE[${CXR_IGRID}]=$(common.runner.evaluateRule "$CXR_3D_CLD_FILE_RULE_CAMXV6" false CXR_3D_CLD_FILE_RULE_CAMXV6 true $create_missing_dirs)
		#Checks
		CXR_CHECK_THESE_INPUT_FILES="$CXR_CHECK_THESE_INPUT_FILES ${CXR_LANDUSE_GRID_INPUT_ARR_FILES[${CXR_IGRID}]} ${CXR_ZP_GRID_INPUT_ARR_FILES[${CXR_IGRID}]} ${CXR_WIND_GRID_INPUT_ARR_FILES[${CXR_IGRID}]} ${CXR_TEMP_GRID_INPUT_ARR_FILES[${CXR_IGRID}]} ${CXR_VAPOR_INPUT_ARR_FILES[${CXR_IGRID}]} ${CXR_CLOUD_GRID_INPUT_ARR_FILES[${CXR_IGRID}]} ${CXR_KV_GRID_INPUT_ARR_FILES[${CXR_IGRID}]} ${CXR_EMISS_INPUT_ARR_FILES[${CXR_IGRID}]} ${CXR_2D_SFC_CAMXV6_INPUT_ARR_FILE[${CXR_IGRID}]} ${CXR_3D_MET_CAMXV6_INPUT_ARR_FILE[${CXR_IGRID}]} ${CXR_2D_MET_CAMXV6_INPUT_ARR_FILE[${CXR_IGRID}]} ${CXR_3D_KV_CAMXV6_INPUT_ARR_FILE[${CXR_IGRID}]} ${CXR_3D_CLD_CAMXV6_INPUT_ARR_FILE[${CXR_IGRID}]}"

		# These are used to prevent overwriting of existing files
		# Output files must not be decompressed! 
		CXR_AVG_OUTPUT_ARR_FILES[${CXR_IGRID}]=$(common.runner.evaluateRule "$CXR_AVG_FILE_RULE" false CXR_AVG_FILE_RULE false $create_missing_dirs)
		CXR_DEPN_OUTPUT_ARR_FILES[${CXR_IGRID}]=$(common.runner.evaluateRule "$CXR_DEPN_FILE_RULE" false CXR_DEPN_FILE_RULE false $create_missing_dirs)
...
################################################################################
{
	...
	then
		# Up tp CAMx-v5.40: veritcal nesting option is available
		echo " Number_of_Layers(1)  = ${CXR_NUMBER_OF_LAYERS[1]}," >> ${CXR_MODEL_CTRL_FILE}
	else
		# Starting from CAMx-v6.00: vertical nesting option is not available anymore 
		echo " Number_of_Layers  = ${CXR_NUMBER_OF_LAYERS[1]}," >> ${CXR_MODEL_CTRL_FILE}		 
	fi
	...
	do
		...
		if [[ "$(common.math.compareVersions "$CXR_MODEL_VERSION" "6.00" )" -ne 1 ]]
		then
		    # Starting from CAMx-v6.00 veritcal nesting is not available anymore
			echo "CAMx version:" $CXR_MODEL_VERSION "doesn't support vertical nesting anymore."
		else
	...
	then
		...
		if [[ "$(common.math.compareVersions "$CXR_MODEL_VERSION" "6.00" )" -ne 1 ]]
		then 
			# Starting with CAMx 6.00, all cloud and haze adjustments are made using the in-line TUV
			echo "CAMx version:" $CXR_MODEL_VERSION "uses in-line TUV: cloud adjust flag not needed" 
		else
		...
		then
			if [[ ${CXR_MODEL_VERSION:0:1} -ge 6 ]]
			then
				# Starting with CAMx 6.00, all cloud and haze adjustments are made using the in-line TUV
				echo "CAMx version:" $CXR_MODEL_VERSION "uses in-line TUV: aerosol adjust flag not needed" 
			else 
	...
	if [[ "$(common.math.compareVersions "$CXR_MODEL_VERSION" "6.00" )" -ne 1 ]]
	then
		# Starting with CAMx 6.00, this is not needed anymore
		echo "CAMx version:" $CXR_MODEL_VERSION "does not need Staggered_winds flag in the namelist file anymore"  
	else
	...
	then
		# Starting with CAMx 6.00, this is not needed anymore
		echo "CAMx version:" $CXR_MODEL_VERSION "does not need number of species explicitly set in the namelist file anymore"
		echo "But we calculate it in the configuration file of the Runs to allow CAMxRunner to check limits and other stuffs "		
	else
    ...
    if [[ "$(common.math.compareVersions "$CXR_MODEL_VERSION" "6.00" )" -ne 1 ]]
	then
		# Starting with CAMx 6.00, we just need on ozone map.
		echo " Ozone_Column    = '${CXR_O3MAP_INPUT_FILE}'," >> ${CXR_MODEL_CTRL_FILE}
	else
	...
	do
		...
		then
			...
			echo " Surface_Grid(${iGrid}) = '${CXR_2D_SFC_CAMXV6_INPUT_ARR_FILE[${iGrid}]}'," >> ${CXR_MODEL_CTRL_FILE} 
 			echo " Met3D_Grid(${iGrid})   = '${CXR_3D_MET_CAMXV6_INPUT_ARR_FILE[${iGrid}]}'," >> ${CXR_MODEL_CTRL_FILE} 
 			echo " Met2D_Grid(${iGrid})   = '${CXR_2D_MET_CAMXV6_INPUT_ARR_FILE[${iGrid}]}'," >> ${CXR_MODEL_CTRL_FILE} 
			echo " Vdiff_Grid(${iGrid})   = '${CXR_3D_KV_CAMXV6_INPUT_ARR_FILE[${iGrid}]}'," >> ${CXR_MODEL_CTRL_FILE}
 			echo " Cloud_Grid(${iGrid})   = '${CXR_3D_CLD_CAMXV6_INPUT_ARR_FILE[${iGrid}]}'," >> ${CXR_MODEL_CTRL_FILE} 
			echo " Emiss_Grid(${iGrid})   = '${CXR_EMISS_INPUT_ARR_FILES[${iGrid}]}'," >> ${CXR_MODEL_CTRL_FILE}
		fi	
	done
