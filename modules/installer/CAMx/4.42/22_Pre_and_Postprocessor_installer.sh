	...
	then
		...
		do
			...
			then
				...
				cd $SRC_DIR || cxr_main_die_gracefully "Could not change to $SRC_DIR"
				
				# Clean up whatever there was
				make clean DESTINATION="${CXR_BIN_DIR}/${CXR_MODEL}/${CXR_MODEL_VERSION}"
				
				# Make it!
				make FC=${CXR_FORTRAN_COMPILER_EXEC} PLATFORM=$CXR_CURRENT_PLATFORM DESTINATION="${CXR_BIN_DIR}/${CXR_MODEL}/${CXR_MODEL_VERSION}" || cxr_main_die_gracefully "The compilation did not complete successfully"
			fi
		done
		
		cd $CXR_RUN_DIR || return $CXR_RET_ERROR
		
		cxr_main_logger -a "${FUNCNAME}" "Converters compiled."
