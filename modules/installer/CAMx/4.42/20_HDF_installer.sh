...
################################################################################
{
	...
	then
		...
		mkdir -p $MYLIBDIR/zlib || main_dieGracefully "could not create $MYLIBDIR/zlib"
		mkdir -p $MYLIBDIR/hdf || main_dieGracefully "could not create to $MYLIBDIR/hdf"
		
		mkdir -p $MYLIBDIR/netcdf || main_dieGracefully "could not create $MYLIBDIR/netcdf"
		mkdir -p $MYLIBDIR/ioapi || main_dieGracefully "could not create $MYLIBDIR/ioapi"
		...
		# Go to location
		cd $MYLIBDIR/zlib || main_dieGracefully "could not change to $MYLIBDIR/zlib"
	
		# Download
		${CXR_WGET_EXEC} ${CXR_ZLIB_TAR_LOC} -O ${CXR_ZLIB_TAR} || main_dieGracefully "could not download ${CXR_ZLIB_TAR_LOC}"
		
		# Untar 
		tar xvzf ${CXR_ZLIB_TAR}
		
		# Go to directory
		cd ${CXR_ZLIB_TAR_DIR} || main_dieGracefully "could not change to $CXR_ZLIB_TAR_DIR"
		
		# Configure
		./configure --shared --prefix=$MYLIBDIR/zlib || main_dieGracefully "Configure for zlib failed"
		
		# Compile
		make clean
		make && make install || main_dieGracefully "make for zlib failed"
		
		# Clean up
		cd $MYLIBDIR/zlib || main_dieGracefully "Could not move up"
		rm ${CXR_ZLIB_TAR}
		...
		# Go to location
		cd $MYLIBDIR/hdf || main_dieGracefully "could not change to $MYLIBDIR/hdf"
	
		# Download
		${CXR_WGET_EXEC} ${CXR_HDF_TAR_LOC} -O ${CXR_HDF_TAR} || main_dieGracefully "could not download ${CXR_HDF_TAR_LOC}"
		
		# Untar 
		tar xvzf ${CXR_HDF_TAR}
		
		# Go to directory
		cd ${CXR_HDF_TAR_DIR} || main_dieGracefully "could not change to $CXR_HDF_TAR_DIR"
		
		# Configure
		./configure --with-zlib=$MYLIBDIR/zlib --prefix=$MYLIBDIR/hdf || main_dieGracefully "Configure for hdf failed"
		
		# Compile
		make clean
		make && make install || main_dieGracefully "make for hdf failed"
		
		# Clean up
		cd $MYLIBDIR/hdf || main_dieGracefully "Could not move up"
		rm ${CXR_HDF_TAR}
		...
		# Go to location
		cd $MYLIBDIR/netcdf || main_dieGracefully "could not change to $MYLIBDIR/netcdf"
		
		# Download
		${CXR_WGET_EXEC} ${CXR_NETCDF_TAR_LOC} -O ${CXR_NETCDF_TAR} || main_dieGracefully "could not download ${CXR_NETCDF_TAR_LOC}"

		# Untar 
		tar xvzf ${CXR_NETCDF_TAR}
		
		# Go to directory
		cd ${CXR_NETCDF_TAR_DIR} || main_dieGracefully "could not change to $CXR_NETCDF_TAR_DIR"
		
		./configure --enable-netcdf-4 --with-hdf5=$MYLIBDIR/hdf  --with-zlib=$MYLIBDIR/zlib --prefix=$MYLIBDIR/netcdf

		# Compile
		make clean
		make  && make install && make check || main_dieGracefully "make for netcdf failed"
		
		# Clean up
		cd $MYLIBDIR/netcdf || main_dieGracefully "Could not move up"
		rm ${CXR_NETCDF_TAR}
		...
		# Go to location
		cd $MYLIBDIR/ioapi || main_dieGracefully "could not change to $MYLIBDIR/ioapi"
		
		# Download
		${CXR_WGET_EXEC} ${CXR_IOAPI_TAR_LOC} -O ${CXR_IOAPI_TAR} || main_dieGracefully "could not download ${CXR_IOAPI_TAR_LOC}"

		# Untar 
		tar xvzf ${CXR_IOAPI_TAR}
		
		# Go to directory
		cd ${CXR_IOAPI_TAR_DIR} || main_dieGracefully "could not change to $CXR_IOAPI_TAR_DIR"
		...
		make clean && make all && make install || main_dieGracefully "make for ioapi failed"
		
		# Clean up
		cd $MYLIBDIR/ioapi || main_dieGracefully "Could not move up"
		rm ${CXR_IOAPI_TAR}
