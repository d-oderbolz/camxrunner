...
c
      parameter(VEROSAT='Ozone Source Apportionment Technology, OSAT 070
     &603')
     ...
     &PCA 070603')
      parameter(VERDDM='De-coupled Direct Method, DDM 070603')
      parameter(VERPA='Process Anaylsis, PA 070603')
      parameter(VERRTRAC='Reactive Tracers, RTRAC 070603')
      parameter(VERPSAT='Particulate Source Apportionment Tracers, PSAT
     & 070603')
c
c-----------------------------------------------------------------------
...
c    iowsmp   I  unit number of the RTRAC sampling grid concentration file
c
      integer iormap(MXGRID)
      integer iorrcp
      ...
      integer iortc
      integer iorchm
      integer iortem(MXGRID,MXTEMF)
      integer iortpt(MXTEMF)
      integer iowcon(IDXCRS:IDXFIN)
      integer iowsfc(MXGRID)
      integer iowrcp
      integer iowsmp(MXSAMPLE)
c
      common /ptiount/ iormap, iorrcp, iorini, ioric, iorbc,
     &                 iortc, iorchm, iortem, iortpt, iowcon,
     &                 iowsfc, iowrcp, iowsmp
c
c-----------------------------------------------------------------------
...
c   licfil   L   flag to determine if IC file was supplied for RTRAC 
c   bcfil    C   filename for the BC file for DDM or RTRAC
c   lbcfil   L   flag to determine if BC file was supplied for RTRAC 
c   tcfil    C   filename for the TOPCONC file for DDM or RTRAC
c   ltcfil   L   flag to determine if TC file was supplied for RTRAC 
...
c   smpfil   C   filename for the RTRAC sampling grid concentration file
...
c
      ...
      character*200 sfcfil(MXGRID)
      logical       lsfcfl
      character*200 avgfil
      character*200 smpfil(MXSAMPLE)
      logical       lfirst
      character*80 verson
c     
      common /filchr/ flrtsa, mapfil, rcpfil, icfil, bcfil, tcfil, 
     &                chmfil, inifil, temfil, tptfil, confil,
     &                sfcfil, avgfil, smpfil, verson 
      common /fildat/ lmapfl, lrcpfil, licfil, lbcfil, ltcfil, ltemfl, 
     &                ltptfl, lsfcfl, lfirst
c
c-----------------------------------------------------------------------
...
c   RTSMP2D  I   maximum number of entries in the 2-D sampling arrays
c                
c
      integer   MXSA2D
      integer   MXSA3D
...
c
      ...
      parameter( RTSMP2D = MXCOLS*MXROWS*MXTRSP*MXSAMPLE )
c
c-----------------------------------------------------------------------
...
c    ITRPN4   I   index of particulate amonnia
c    ITRALK   I   index of Alkane/Parraffin Secondary Organic Precursors
c    ITRARO   I   index of Aromatic Secondary Organic Aerosol Precursors
c    ITRCRE   I   index of Cresol Secondary Organic Aerosol Precursors
c    ITRTRP   I   index of Biogenic olefin Secondary Organic Precursors
...
c   ipufmap   I   region from which each PiG puff originated
c   ipufgrp   I   group from which each PiG puff originated
c   lpigsa    L   flag to determine if source is a PiG source
c                   
      integer   nreles
      integer   ntrtim
      integer   ipigmap(MXPTSRC)
      integer   ipiggrp(MXPTSRC)
      integer   ipufmap(MXPIG)
      integer   ipufgrp(MXPIG)
      real      saemis(MXSA2D)
...
c
      common /empdat/  saemis, sapnts, ipigmap, ipiggrp, ipufmap,
     &                 ipufgrp, xlocpt, ylocpt, lreles, nreles, 
     &                 ntrtim, lpigsa
c
c-----------------------------------------------------------------------
...
c
      common /ptgdat/ ptconc, ptavrg, ptloft, ptvdep
c
c-----------------------------------------------------------------------
...
c
      ...
      logical      lsagas(MXTRSP)
      real         trspmap(MXSPEC,MXALCLS)
      real         fluxmap(MXSPEC,MXALCLS)
      real         yratmap(MXSPEC,MXALCLS)
      logical      lusespc(MXSPEC)
c
      common /ptnchr/ ptname, clsnam
      common /ptndat/ nsaspc, ntrcls, iptcls, nptcls, ipttrc, npttrc, 
     &                idxcls, idxipt, npttim, ntotsp, iemcls, nemcls, 
     &                ipttim, iemtim, nbdic, lsamap, lsagas, trspmap, 
     &                fluxmap, yratmap, lusespc
c
c-----------------------------------------------------------------------
...
c
     ...
     &                ipekcl, recepx, recepy, conrcp, iwalbg, iwalnd, 
...
c 
      integer*4  MXSPALL
      parameter( MXSPALL = max(MXTRSP,MXSPEC) )
c
      integer   idxems(MXGRID,0:MXTEMF,MXSPALL)
      integer   nspcem(MXGRID,0:MXTEMF)
      integer   idxpts(0:MXTEMF,MXSPALL)
...
c
     ...
     &                lo3sp, crbnum, mwspec, rkohrt, rmirrt, wtkoh, 
     &                wtmir, yrates
c
c-----------------------------------------------------------------------
...
c
c-----------------------------------------------------------------------
c  Parameters for Rtrac/PiG sampling grids:
c-----------------------------------------------------------------------
c
...
c
      logical lsmptrc
      integer iprtsmp(MXSAMPLE)
      real    puffrt(MXTRSP,MXRECTR,MXPIG),rtsmpcnc(RTSMP2D)
c
      common /rtracsmp/ lsmptrc,iprtsmp,puffrt,rtsmpcnc
