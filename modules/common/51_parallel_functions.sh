...
################################################################################
{
	...
	do
		...
		then
			...
			main_log -v "$FUNCNAME" "$indep equals $dep"
		fi
	...
	then
		main_log -e "$FUNCNAME" "Could not visualize the dependencies."
	else	
		main_log -a "$FUNCNAME" "You find a visualisation of the modules dependencies in the file ${output_file}"
	fi
}
...
################################################################################
{
	...
	main_log -v "${FUNCNAME}"  "Found $task_count open tasks"
...
################################################################################
{
	...
	then
		main_log -v "${FUNCNAME}"  "Currently, no new tasks are being assigned. Probably the model is running"
	...
	if [[ "$task_count" -eq 0  ]]
	then
		main_log "${FUNCNAME}" "All tasks have been processed, notifing system..."
		...
		main_log -v "${FUNCNAME}"  "There are $task_count unfinished tasks - we choose the top one."
	fi
	...
	then
		# No task!
		main_log -v "${FUNCNAME}" "Did not find any task..."
		echo ""
		...
		main_log -v "${FUNCNAME}"  "New descriptor is $new_descriptor_name"
...
################################################################################
{
	...
	main_log -v "${FUNCNAME}"  "cxr_common_parallel_worker (task_pid: $task_pid) changed its state to waiting"
...
################################################################################
{
	...
	main_log -v "${FUNCNAME}"  "cxr_common_parallel_worker (task_pid: $task_pid) changed its state to working"
...
################################################################################
{
	...
	if [[ "$node" != "$(uname -n)"  ]]
	then
		main_log "${FUNCNAME}" "Strange: $task_pid seems to run on $node rather than on $(uname -n)"
...
################################################################################
{
	...
	main_log -a -B "${FUNCNAME}" "parallel worker (task_pid $task_pid) starts..."
	...
	then
		...
		main_log "${FUNCNAME}" "This cxr_common_parallel_worker will use its own logfile: ${CXR_LOG}"
	fi
	...
	do
		...
		then
			...
			main_log -a "${FUNCNAME}" "New task received: $new_task_descriptor"
			...
			main_log -v "${FUNCNAME}"  "task: $task\nDAY_OFFSET: $day_offset\nEXCLUSIVE: $exclusive"
			
			#Reserve resources if needed
			if [[ "$exclusive" == true  ]]
			then
				main_log "${FUNCNAME}" "This task needs exclusive access, we suspend the assignment of new tasks temporarily"
			...
			main_log -B "${FUNCNAME}"  "cxr_common_parallel_worker $task_pid assigned to $task for $CXR_DATE"
			
			# Before loading a new module, remove old meta variables
			unset ${!CXR_META_MODULE*}
			
			# Export the module name
			CXR_META_MODULE_NAME=$(main_getModuleName $task)
			...
			if [[ "$exclusive" == true  ]]
			then
				main_log "${FUNCNAME}" "Activating the assignment of new tasks again."
			...
			then
				main_log -v "${FUNCNAME}" "cxr_common_parallel_worker $task_pid did not receive an assignment - there seem to be too many workers (starvation)"
...
################################################################################
{
	main_log "${FUNCNAME}" "We create now $1 cxr_common_parallel_worker threads"
...
################################################################################
{
	main_log "${FUNCNAME}" "We remove all workers now."
...
################################################################################
{
		main_log "${FUNCNAME}" "Entering a wait loop (the work is carried out by backgound processes. I check every $CXR_WAITING_SLEEP_SECONDS seconds if all is done.)"
		...
		main_log -B "${FUNCNAME}"  "The Continue file is gone, all workers will stop asap."
...
################################################################################
{
	...
	main_log -a "${FUNCNAME}" "Creating a list of dependencies..."
		
	cxr_common_parallel_create_dep_list "$dep_file"
	
	main_log -a "${FUNCNAME}" "Ordering tasks..."
	...
	main_log -a "${FUNCNAME}" "This run consists of $(( $current_id -1 )) tasks."
}
