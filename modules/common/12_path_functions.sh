...
################################################################################
{
	...
	then
		...
		main_log -e "$FUNCNAME" "No valid filename passed!"
		mtime=0
	fi
...
################################################################################
{
	...
	then
		...
		main_log -e "$FUNCNAME" "the file command reported an error for $file"
		echo ""
	else
		echo "$filetype"
	fi
...
################################################################################
{
	...
	if [[ ! -f "$1"  ]]
	then
		main_log -w "$FUNCNAME" "The file $1 does not exist!"
		echo 0
...
################################################################################
{
	...
	if [[ ! -f "$filename" ]]
	then
		main_log -e "$FUNCNAME" "$filename still does not exist, timeout reached."
		echo false
	else
		main_log -v "$FUNCNAME" "$filename exists now."
		echo true
	fi
}
...
################################################################################
{
	...
	then
		main_log -e "$FUNCNAME" "$filename still seems to grow."
		echo false
	else
		main_log -v "$FUNCNAME" "$filename size is stable now."
		echo true
	fi
}
			...
			do
				...
				then
					...
					then
						...
						then
							...
							then
								...
								main_log -v "$FUNCNAME" "Pattern ${CXR_COMPRESS_OUTPUT_PATTERN} did not match $line, will not compress this file"
							fi
						fi
					else
					...
					then
						...
						then
							main_log -v "$FUNCNAME" "Compressing ${filename} using ${CXR_COMPRESSOR_EXEC}"
							"${CXR_COMPRESSOR_EXEC}" "${filename}"
						else
							main_log -v "$FUNCNAME" "${filename} is smaller than CXR_COMPRESS_THRESHOLD_MB (${CXR_COMPRESS_THRESHOLD_MB})."
						fi
					fi
				else
					#File empty
					main_log -w "$FUNCNAME" "The output file ${filename} is empty, no compression attempted!"
				fi
			done < "${CXR_INSTANCE_FILE_OUTPUT_LIST}" # Loop over lines
		else
			main_log -w "$FUNCNAME" "The output file list ${CXR_INSTANCE_FILE_OUTPUT_LIST} is empty!"
		fi
	else
		main_log "$FUNCNAME" "Will not compress any output files (either dry run or CXR_COMPRESS_OUTPUT is false.)"
	fi
}
...
################################################################################
{
	...
	then
		main_log -v -B "$FUNCNAME" "Testing compression on $(basename ${input_file})..."
		...
		then
			...
			if [[ -s "$tempfile" ]]
			then
				main_log -v "$FUNCNAME" "File ${input_file} was already decompressed into $tempfile"
				...
				main_log -v "$FUNCNAME" "File ${input_file} was already decompressed into $tempfile but for some reason that file is empty!"
		...
		do
			...
			main_log -v "$FUNCNAME" "Looking for $comp_file"
			...
			then
				...
				case $filetype in
					...
					bzip2)
						main_log -a "$FUNCNAME" "${input_file} is bzip2-compressed. Using $CXR_BUNZIP2_EXEC to decompress..."
						$CXR_BUNZIP2_EXEC -c "$comp_file" > $tempfile
						;;
						
					gzip)
						main_log -a "$FUNCNAME" "${input_file} is gzip-compressed. Using $CXR_GUNZIP_EXEC to decompress..."
						"$CXR_GUNZIP_EXEC" -c "$comp_file" > $tempfile
						;;
				
					zip)
						main_log -a "$FUNCNAME" "${input_file} is zip-compressed. Using $CXR_GUNZIP_EXEC to decompress..."
						"$CXR_GUNZIP_EXEC" -S .zip -c "$comp_file" > $tempfile
						;;
				
					*)
						main_log -e "$FUNCNAME" "Compressed file type $filetype not supported"
						;;
				esac
				...
				then
					...
					main_log -e "${FUNCNAME}:${LINENO} - File ${comp_file} could not be decompressed by $DECOMP"
				fi
		...
		then
			...
			# Was not compressed
			main_log -v "$FUNCNAME" "File ${input_file} is not compressed."
			echo "$input_file"
		fi
		
	else
		# We do not consider compressed files
		main_log -v "$FUNCNAME" "We do not try to decompress files, CXR_DETECT_COMPRESSED_INPUT_FILES is false"
		echo "$input_file"
...
################################################################################
{
	if [[ $# -ne 1 ]]
	then
		main_log -e "${FUNCNAME}" "Could not determine FS type - no path passed!"
		echo ""
	...
	if [[ "$num_lines" -eq 3  ]]
	then
		main_log -v "${FUNCNAME}"  "FS type of $1 was determined to be ${df_array[0]}. If this is strange, check your df -T output!"
		echo "${df_array[0]}" 
	elif [[ "$num_lines" -eq 2  ]]
	then
		main_log -v "${FUNCNAME}"  "FS type of $1 was determined to be ${df_array[1]}. If this is strange, check your df -T output!"
		echo "${df_array[1]}" 
	else
		main_log -e "${FUNCNAME}" "Could not determine FS type of $1. Check your df -T output!"
		echo ""
	fi
}
...
################################################################################
{
	...
	case $fs in
	afs)
			main_log -v "${FUNCNAME}" "Directory $1 seems to be on AFS. Getting AFS quota..."
