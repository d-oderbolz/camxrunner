...
################################################################################
{
	...
	echo "$FILENAME" >> "$CXR_DUMMY_FILE_LIST"
...
################################################################################
{
	...
	then
		local temp_list="$CXR_TEMP_LONGTERM_FILE_LIST"
	else
		local temp_list="$CXR_TEMP_FILE_LIST"
	fi
...
################################################################################
{
	...
	if [ "$long_lifetime" == true ]
	then
		local temp_lists="$CXR_TEMP_LONGTERM_FILE_LIST $CXR_TEMP_FILE_LIST"
		cxr_main_logger $FUNCNAME "Removing all tempfiles (if any)"
	else
		local temp_lists="$CXR_TEMP_FILE_LIST"
...
# CXR_LOCK_LIST. This allows to release the locks later when we exit from the program
...
#
# Releases a lock and removes it from CXR_LOCK_LIST.
#
# Recommended call:
...
################################################################################
{
	...
	sed "/^$LOCK\$/d" $CXR_LOCK_LIST > $NEW_LIST
	
	# Exchange tempfile and new file
	mv $NEW_LIST $CXR_LOCK_LIST
...
#
# Releases all locks held by iterating over CXR_LOCK_LIST.
#
#
# Parameters:
# $1 - the name of the lock to release
