	...
	then
		...
		case $(basename ${CXR_STATION_PROC_INPUT_FILE} .pro) in
			...
			extract_nabel_stations)
				...
				cat <<-EOF > $exec_tmp_file
				.run $(basename ${CXR_STATION_PROC_INPUT_FILE})
				$(basename ${CXR_STATION_PROC_INPUT_FILE} .pro),'${CXR_MODEL_INPUT_FILE}','${CXR_STATION_OUTPUT_DIR}',${CXR_DAY},${CXR_MONTH},${CXR_YEAR},${CXR_MODEL_HOUR},${species_array},${stations_array},$(common.runner.getZ $CXR_IGRID),is_binary=1
				exit
				EOF
				;;
