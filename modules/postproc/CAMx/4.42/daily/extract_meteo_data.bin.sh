...
CXR_META_MODULE_DESCRIPTION="Extracts meteo data for defined measurement stations. Currently, only PBL height is implemented"
...
CXR_META_MODULE_REQ_SPECIAL="exec|diagnose_pbl exec|idl"
...
################################################################################
{
	...
	Z=$(common.runner.getZ ${CXR_IGRID})
	...
	#Checks
	CXR_CHECK_THESE_INPUT_FILES="$CXR_CHECK_THESE_INPUT_FILES $CXR_MODEL_INPUT_FILE $CXR_ZP_GRID_FILE $CXR_KV_GRID_INPUT_ARR_FILE $CXR_TEMP_GRID_FILE"
	...
	do
		...
		CXR_STATION_OUTPUT_ARR_FILES[${iStation}]=$(common.runner.evaluateRule "$CXR_METEO_STATION_FILE_RULE" false CXR_METEO_STATION_FILE_RULE false)
...
################################################################################
# Function: extract_meteo_data
#	
# Currently extracts the PBL data (innermost domain) from the locations of the stations
...
################################################################################
{
	...
	then
		...
		# Our postprocessor creates a faked binary average file from which we then extract
		fake_avg_file=$(common.runner.createTempFile $FUNCNAME)
...
nz                 |${Z}
		...
		case $(basename ${CXR_STATION_PROC_INPUT_FILE} .pro) in
			...
			extract_arpa_stations)
				main.log -w "Sorry, extracgtion of PBL only works with extract_nabel_stations"
				;;
			extract_nabel_stations)
				
				cat <<-EOF > $exec_tmp_file
				.run $(basename ${CXR_STATION_PROC_INPUT_FILE})
				$(basename ${CXR_STATION_PROC_INPUT_FILE} .pro),'${fake_avg_file}','${CXR_STATION_OUTPUT_DIR}',${write_header},${CXR_DAY},${CXR_MONTH},${CXR_YEAR},${CXR_MODEL_HOUR},${species_array},${stations_array},$(common.runner.getZ $CXR_IGRID),is_binary=1
				exit
				EOF
				;;
