...
################################################################################
{
	...
	then
		main_log -w "${FUNCNAME}"  "This module is susceptible to limited processing, because it creates links rather than files.\nIf you use compressed input files, the temporary files into which we decompress wil be deleted.\nUse \n \t ${CXR_CALL} -L \nto avoid this."
	fi
		...
		do
			...
			do
				...
				then
					if [[ "$CXR_DRY" == "false"  ]]
					then
						main_log "${FUNCNAME}"  "Linking ${current_base} to ${current_file}..."
						ln -s ${current_file} ${current_base}
					else
						main_log "${FUNCNAME}"  "This is a dry run, do not create a link for output processing"
					fi
				fi
			done
		done
