...
################################################################################
{
	...
	then
		...
		main_log "${FUNCNAME}" "Working in $CXR_AQMFAD_OUTPUT_DIR"
		...
		do
			...
			do
				...
				main_log "${FUNCNAME}" "Converting ${input_file} to ${output_file} using ${converter}..."
				...
				then
					...
					then
						# Delete it
						main_log "${FUNCNAME}"  "File ${output_file} exists - since you run with the -f option, if will be deleted now"
						rm -f "$output_file"
					else
						# Skip it
						main_log "${FUNCNAME}"  "File ${output_file} exists - file will skipped."
						continue
					fi
				fi
				...
				main_log -v "${FUNCNAME}" "${converter} ${input_file} ${output_file} ${options} ${xdim} ${ydim} ${zdim} 0 "
