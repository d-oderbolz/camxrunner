...
################################################################################
{
	...
	then
		...
		do
			...
			main_log "${FUNCNAME}"  "Creating links in the $CXR_AQMFAD_OUTPUT_DIR directory..."
			...
			then
				...
				then
					...
					main_log -e "${FUNCNAME}" "Cannot find the emission file ${CXR_EMISSION_GRID_ASC_INPUT_ARR_FILES[${i}]}"
				fi
			fi
			...
			main_log "${FUNCNAME}"  "Running emifad on grid ${i}..."
			main_log "${FUNCNAME}"  "${CXR_EMIFAD_EXEC} fi_emi=$(basename ${CXR_EMISSION_GRID_ASC_INPUT_ARR_FILES[${i}]}) fi_terrain=$(basename ${CXR_TERRAIN_GRID_ASC_INPUT_ARR_FILES[${i}]})"
